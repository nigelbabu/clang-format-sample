...
int dht_removexattr2(xlator_t *this, xlator_t *subvol, call_frame_t *frame,
                     int ret);

int dht_setxattr2(xlator_t *this, xlator_t *subvol, call_frame_t *frame,
                  int ret);

int dht_rmdir_readdirp_do(call_frame_t *readdirp_frame, xlator_t *this);

int dht_common_xattrop_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
int add_opt(char **optsp, const char *opt) {
    ...
    if (*optsp == NULL)
        ...
        if (newopts) sprintf(newopts + oldsize, ",%s", opt);
    }
    if (newopts == NULL) {
...
out:
    if (old_choice) GF_FREE(old_choice);
    if (new_choice) GF_FREE(new_choice);
    if (full_choice) GF_FREE(full_choice);
    ...
    if (strcmp(key, GF_AFR_SBRAIN_STATUS) == 0) {
        ...
        if (!strncmp(key, "user.", strlen("user."))) {
            ...
            if (!ret && dict_data && value) {
                ...
                if (!ret) gf_msg_debug("dht", 0, "xattr mismatch for %s", key);
            }
        }
    }
...
unlock:
    UNLOCK(&inode->lock);
    if (ret && ctx_free) GF_FREE(ctx);
    return ret;
}

/*Code to get mds subvol from inode ctx */

int dht_inode_ctx_mdsvol_get(inode_t *inode, xlator_t *this,
                             xlator_t **mdsvol) {
    dht_inode_ctx_t *ctx = NULL;
    int ret = -1;

    if (!mdsvol) return ret;
...
int dht_lookup_selfheal_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
out:
    if (local && local->mds_heal_fresh_lookup) DHT_STACK_DESTROY(frame);
    return 0;
}
...
/* Common function call by revalidate/selfheal code path to populate
   ...
   penalty.
*/
int dht_common_mark_mdsxattr(call_frame_t *frame, int *errst,
    ...
    if (!dict_get(local->xattr, conf->mds_xattr_key)) {
        ...
        */
        if (mark_during_fresh_lookup) {
            ...
            if (!copy_local->inode) copy_local->inode = inode_ref(local->inode);
...
out:
    if (xattrs) dict_unref(xattrs);
    return ret;
}

int dht_discover_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    LOCK(&frame->lock);
    {
        ...
        */
        ...
        if (local->xattr == NULL) {
            ...
            if (is_dir) dht_aggregate_xattr(local->xattr, xattr);
        }

        if (local->inode == NULL) local->inode = inode_ref(inode);
...
out:
    ...
    if (is_last_call(this_call_cnt)) DHT_STACK_DESTROY(frame);
...
int dht_lookup_dir_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    if (local->gfid) gf_uuid_unparse(local->gfid, gfid_local);
...
int dht_revalidate_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    LOCK(&frame->lock);
    {
        ...
        if (is_dir) {
            ...
            ret =
                dht_layout_dir_mismatch(this, layout, prev, &local->loc, xattr);
            if (ret != 0) {
...
int dht_lookup_linkfile_create_cbk(call_frame_t *frame, void *cookie,
...
unwind:
    ...
    if (local->linked == _gf_true) dht_linkfile_attr_heal(frame, this);
...
int dht_lookup_unlink_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
int dht_lookup_unlink_of_false_linkto_cbk(call_frame_t *frame, void *cookie,
...
int dht_lookup_unlink_stale_linkto_cbk(call_frame_t *frame, void *cookie,
                                       xlator_t *this, int op_ret, int op_errno,
    ...
    if (local && local->loc.path) path = local->loc.path;
...
int dht_lookup_everywhere_done(call_frame_t *frame, xlator_t *this) {
    ...
    if (hashed_subvol || local->need_lookup_everywhere) {
        ...
        if (found_non_linkto_on_hashed) goto preset_layout;
    }
...
int dht_lookup_everywhere_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    {
        if (op_ret == -1) {
            if (op_errno != ENOENT) local->op_errno = op_errno;
            goto unlock;
        }
        ...
        is_linkfile =
            check_is_linkfile(inode, buf, xattr, conf->link_xattr_name);
        ...
        if (!local->cached_subvol) gf_uuid_copy(local->gfid, buf->ia_gfid);

        if (is_dir) {
            local->dir_count++;

            gf_msg_debug(this->name, 0, "found on %s directory %s",
                         subvol->name, loc->path);
        } else {
            local->file_count++;
            ...
            if (!local->cached_subvol) {
                ...
                gf_msg(
                    this->name, GF_LOG_WARNING, 0, DHT_MSG_FILE_ON_MULT_SUBVOL,
    ...
    if (is_linkfile) {
        ...
        if (local->hashed_subvol && local->hashed_subvol == subvol) {
            ...
            if (ret) {
                 ...
                 */
                FRAME_SU_DO(frame, dht_local_t);
                STACK_WIND(frame, dht_lookup_unlink_cbk, subvol,
                           subvol->fops->unlink, loc, 0, dict_req);
...
int dht_lookup_everywhere(call_frame_t *frame, xlator_t *this, loc_t *loc) {
    ...
    if (!local->inode) local->inode = inode_ref(loc->inode);
...
out:
    if (populate_loc.inode) loc_wipe(&populate_loc);
    return hash_subvol;
}
    ...
    if (ENTRY_MISSING(op_ret, op_errno)) {
        ...
        if (!conf->defrag && conf->lookup_optimize && loc->parent) {
            ...
            if ((conf->search_unhashed == GF_DHT_LOOKUP_UNHASHED_AUTO) &&
                (loc->parent)) {
                ret =
                    dht_inode_ctx_layout_get(loc->parent, this, &parent_layout);
                if (ret || !parent_layout) goto out;
...
void dht_check_and_set_acl_xattr_req(inode_t *inode, dict_t *xattr_req) {
    ...
    if (inode->ia_type != IA_IFDIR) return;
...
int dht_lookup(call_frame_t *frame, xlator_t *this, loc_t *loc,
               dict_t *xattr_req) {
    xlator_t *subvol = NULL;
    xlator_t *hashed_subvol = NULL;
    ...
    if (!hashed_subvol) hashed_subvol = dht_subvol_get_hashed(this, loc);
...
int dht_unlink_linkfile_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                            int op_ret, int op_errno, struct iatt *preparent,
                            struct iatt *postparent, dict_t *xdata) {
    dht_local_t *local = NULL;
    xlator_t *prev = NULL;
...
int dht_unlink_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                   int op_ret, int op_errno, struct iatt *preparent,
                   struct iatt *postparent, dict_t *xdata) {
    dht_local_t *local = NULL;
    xlator_t *prev = NULL;
...
static int dht_common_setxattr_cbk(call_frame_t *frame, void *cookie,
                                   xlator_t *this, int32_t op_ret,
...
static int dht_fix_layout_setxattr_cbk(call_frame_t *frame, void *cookie,
                                       xlator_t *this, int32_t op_ret,
                                       int32_t op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_layout_t *layout = NULL;
...
int dht_err_cbk(call_frame_t *frame, void *cookie, xlator_t *this, int op_ret,
...
int dht_common_mds_xattrop_cbk(call_frame_t *frame, void *cookie,
...
/* Code to wind a xattrop call to add 1 on current mds internal xattr
   value
*/
int dht_setxattr_non_mds_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                             int op_ret, int op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    int this_call_cnt = 0;
    int ret = 0;
...
int dht_setxattr_mds_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                         int op_ret, int op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
    call_frame_t *prev = NULL;
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
        if (mds_subvol && (mds_subvol == conf->subvolumes[i])) continue;
        if (local->fop == GF_FOP_SETXATTR) {
...
int dht_xattrop_mds_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
void dht_fill_pathinfo_xattr(xlator_t *this, dht_local_t *local,
                             char *xattr_buf, int32_t alloc_len, int flag,
                             char *layout_buf) {
...
int dht_vgetxattr_alloc_and_fill(dht_local_t *local, dict_t *xattr,
                                 xlator_t *this, int op_errno) {
    int ret = -1;
    char *value = NULL;
    int32_t plen = 0;
...
int dht_vgetxattr_fill_and_set(dht_local_t *local, dict_t **dict,
                               xlator_t *this, gf_boolean_t flag) {
    int ret = -1;
    char *xattr_buf = NULL;
...
int dht_find_local_subvol_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
unlock:
    ...
    if (!is_last_call(this_call_cnt)) goto out;
...
int dht_vgetxattr_dir_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                          int op_ret, int op_errno, dict_t *xattr,
                          dict_t *xdata) {
    int ret = 0;
    dht_local_t *local = NULL;
...
int dht_vgetxattr_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
int dht_linkinfo_getxattr_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                              int op_ret, int op_errno, dict_t *xattr,
                              dict_t *xdata) {
    int ret = 0;
    char *value = NULL;
    ...
    if (op_ret != -1) {
        ...
        if (!ret) {
            ...
            if (!ret) gf_msg_trace(this->name, 0, "failed to set linkinfo");
        }
    }
...
int dht_mds_getxattr_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                         int op_ret, int op_errno, dict_t *xattr,
                         dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
...
int dht_getxattr_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                     int op_ret, int op_errno, dict_t *xattr, dict_t *xdata) {
    int this_call_cnt = 0;
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
...
int32_t dht_getxattr_unwind(call_frame_t *frame, int op_ret, int op_errno,
                            dict_t *dict, dict_t *xdata) {
    DHT_STACK_UNWIND(getxattr, frame, op_ret, op_errno, dict, xdata);
    return 0;
}

int dht_getxattr_get_real_filename_cbk(call_frame_t *frame, void *cookie,
                                       xlator_t *this, int op_ret, int op_errno,
    ...
    LOCK(&frame->lock);
    {
         ...
         * result.
         */
        if (local->xattr) dict_unref(local->xattr);
        local->xattr = dict_ref(xattr);
        ...
        if (xdata) local->xattr_req = dict_ref(xdata);
...
int dht_getxattr_get_real_filename(call_frame_t *frame, xlator_t *this,
...
int dht_marker_populate_args(call_frame_t *frame, int type, int *gauge,
                             xlator_t **subvols) {
    dht_local_t *local = NULL;
    int i = 0;
    dht_layout_t *layout = NULL;
    ...
    for (i = 0; i < layout->cnt; i++) subvols[i] = layout->list[i].xlator;

    return layout->cnt;
}

int dht_getxattr(call_frame_t *frame, xlator_t *this, loc_t *loc,
...
#define DHT_IS_DIR(layout) (layout->cnt > 1)
{
    ...
    if (key && DHT_IS_DIR(layout) &&
        ...
        if (node_uuid_key) GF_FREE(node_uuid_key);
        return 0;
    }
...
int dht_fgetxattr(call_frame_t *frame, xlator_t *this, fd_t *fd,
                  const char *key, dict_t *xdata) {
    xlator_t *subvol = NULL;
    dht_local_t *local = NULL;
    dht_layout_t *layout = NULL;
    ...
    if (fd->inode) gf_uuid_unparse(fd->inode->gfid, gfid);
...
int dht_file_setxattr_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    if (xdata) local->rebalance.xdata = dict_ref(xdata);
...
*/
static int dht_is_user_xattr(dict_t *this, char *key, data_t *value,
                             void *data) {
 ...
 */
int dht_dir_common_setxattr(call_frame_t *frame, xlator_t *this, loc_t *loc,
...

{
    ...
    */
    for (i = 1; xattrs_to_heal[i]; i++)
        if (dict_get(xattr, xattrs_to_heal[i])) uxattr_key_found = _gf_true;
        ...
        for (i = 0; i < conf->subvolume_cnt; i++) {
            ...
            if (fd) {
                STACK_WIND_COOKIE(frame, dht_err_cbk, travvol, travvol,
                                  travvol->fops->fsetxattr, fd, xattr, flags,
                                  xdata);
            } else {
                STACK_WIND_COOKIE(frame, dht_err_cbk, travvol, travvol,
                                  travvol->fops->setxattr, loc, xattr, flags,
                                  xdata);
            }
        }
...
int dht_fsetxattr(call_frame_t *frame, xlator_t *this, fd_t *fd, dict_t *xattr,
    ...
    if (IA_ISDIR(fd->inode->ia_type)) {
        ...
        ret = dht_dir_common_setxattr(frame, this, NULL, fd, xattr, flags,
                                      xdata, &op_errno);
        if (ret) goto err;
    } else {
        local->call_cnt = 1;
...
int dht_checking_pathinfo_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                              int op_ret, int op_errno, dict_t *xattr,
                              dict_t *xdata) {
    int i = -1;
    int ret = -1;
...
int dht_nuke_dir_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
int dht_setxattr(call_frame_t *frame, xlator_t *this, loc_t *loc, dict_t *xattr,
                 int flags, dict_t *xdata) {
    xlator_t *subvol = NULL;
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
    ...
    if (IA_ISDIR(loc->inode->ia_type)) {
        local->hashed_subvol = NULL;
        ret = dht_dir_common_setxattr(frame, this, loc, NULL, xattr, flags,
                                      xdata, &op_errno);
        if (ret) goto err;
    } else {
        local->rebalance.xattr = dict_ref(xattr);
...
int dht_removexattr_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                        int op_ret, int op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    int this_call_cnt = 0;
    xlator_t *prev = NULL;
...
int dht_removexattr(call_frame_t *frame, xlator_t *this, loc_t *loc,
                    const char *key, dict_t *xdata) {
    xlator_t *subvol = NULL;
    int op_errno = -1;
    dht_local_t *local = NULL;
    ...
    if (IA_ISDIR(loc->inode->ia_type)) {
        for (i = 0; i < call_cnt; i++) {
            STACK_WIND_COOKIE(frame, dht_removexattr_cbk,
                              layout->list[i].xlator, layout->list[i].xlator,
                              layout->list[i].xlator->fops->removexattr, loc,
                              key, local->xattr_req);
        }
...
int dht_fremovexattr(call_frame_t *frame, xlator_t *this, fd_t *fd,
                     const char *key, dict_t *xdata) {
    xlator_t *subvol = NULL;
    int op_errno = -1;
    dht_local_t *local = NULL;
    ...
    if (IA_ISDIR(fd->inode->ia_type)) {
        for (i = 0; i < call_cnt; i++) {
            STACK_WIND_COOKIE(frame, dht_removexattr_cbk,
                              layout->list[i].xlator, layout->list[i].xlator,
                              layout->list[i].xlator->fops->fremovexattr, fd,
                              key, local->xattr_req);
        }
...
int dht_fd_cbk(call_frame_t *frame, void *cookie, xlator_t *this, int op_ret,
...
int dht_statfs_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
        ...
        if (local->quota_deem_statfs) {
            switch (action) {
                ...
                case qdstatfs_action_COMPARE:
                    ...
                    cur_usage =
                        local->statvfs.f_blocks - local->statvfs.f_bfree;

                    /* Take the max of the usage from subvols */
                    if (new_usage >= cur_usage) local->statvfs = *statvfs;
                    goto unlock;
...
int dht_opendir(call_frame_t *frame, xlator_t *this, loc_t *loc, fd_t *fd,
                dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
    int op_errno = -1;
...
void dht_populate_inode_for_dentry(xlator_t *this, xlator_t *subvol,
                                   gf_dirent_t *entry,
    ...
    if (layout) dht_layout_unref(this, layout);
...
int dht_readdirp_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    if (local->op_ret < 0) local->op_ret = 0;

    if (!local->layout) local->layout = dht_layout_get(this, local->fd->inode);
    ...
    if (layout == NULL) goto done;

    if (conf->readdir_optimize == _gf_true) readdir_optimize = 1;
    ...
    list_for_each_entry(orig_entry, (&orig_entries->list), list) {
        ...
        if (check_is_dir(NULL, (&orig_entry->d_stat), NULL)) {
            ...
            hashed_subvol =
                methods->layout_search(this, layout, orig_entry->d_name);

            if (prev == hashed_subvol) goto list;
    ...
    list:
        ...
        if (orig_entry->dict) entry->dict = dict_ref(orig_entry->dict);
    ...
    if ((count == 0) || (local && (local->filled < local->size))) {
        ...
        STACK_WIND_COOKIE(frame, dht_readdirp_cbk, next_subvol, next_subvol,
                          ...
                          (local->size - local->filled), next_offset,
                          local->xattr);
        return 0;
    }
...
int dht_readdir_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                    int op_ret, int op_errno, gf_dirent_t *orig_entries,
                    dict_t *xdata) {
    dht_local_t *local = NULL;
    gf_dirent_t *orig_entry = NULL;
...
int dht_do_readdir(call_frame_t *frame, xlator_t *this, fd_t *fd, size_t size,
...
int dht_readdir(call_frame_t *frame, xlator_t *this, fd_t *fd, size_t size,
    ...
    if (conf->use_readdirp) op = GF_FOP_READDIRP;
...
int dht_readdirp(call_frame_t *frame, xlator_t *this, fd_t *fd, size_t size,
                 off_t yoff, dict_t *dict) {
    dht_do_readdir(frame, this, fd, size, yoff, GF_FOP_READDIRP, dict);
    return 0;
}

int dht_fsyncdir_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                     int op_ret, int op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    int this_call_cnt = 0;
    ...
    LOCK(&frame->lock);
    {
        if (op_ret == -1) local->op_errno = op_errno;
...
int dht_fsyncdir(call_frame_t *frame, xlator_t *this, fd_t *fd, int datasync,
                 dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
    int op_errno = -1;
...
int dht_mknod_linkfile_create_cbk(call_frame_t *frame, void *cookie,
                                  xlator_t *this, int32_t op_ret,
                                  int32_t op_errno, inode_t *inode,
                                  struct iatt *stbuf, struct iatt *preparent,
                                  struct iatt *postparent, dict_t *xdata) {
...
int dht_mknod_wind_to_avail_subvol(call_frame_t *frame, xlator_t *this,
...
int32_t dht_mknod_unlock_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                             int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    DHT_STACK_DESTROY(frame);
    return 0;
}

int32_t dht_mknod_finish(call_frame_t *frame, xlator_t *this, int op_ret,
                         int invoke_cbk) {
...
int32_t dht_mknod_lock_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                           int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
...
int dht_refresh_parent_layout_resume(call_frame_t *frame, xlator_t *this,
...
int32_t dht_call_mkdir_stub(call_frame_t *frame, void *cookie, xlator_t *this,
                            int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    call_stub_t *stub = NULL;
...
err:
    ...
    if (parent_disk_layout != NULL) GF_FREE(parent_disk_layout);

    if (parent_layout != NULL) dht_layout_unref(this, parent_layout);

    return -1;
}

int dht_mknod(call_frame_t *frame, xlator_t *this, loc_t *loc, mode_t mode,
...
int dht_symlink(call_frame_t *frame, xlator_t *this, const char *linkname,
...
int dht_unlink(call_frame_t *frame, xlator_t *this, loc_t *loc, int xflag,
               dict_t *xdata) {
    xlator_t *cached_subvol = NULL;
    int op_errno = -1;
    dht_local_t *local = NULL;
...
int dht_link_cbk(call_frame_t *frame, void *cookie, xlator_t *this, int op_ret,
    ...
    if (op_ret == -1) {
        ...
        if (local->linked) {
            ...
            if (cleanup_frame) {
                cleanup_local =
                    dht_local_init(cleanup_frame, &local->loc2, NULL, 0);
...
int dht_link_linkfile_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                          int op_ret, int op_errno, inode_t *inode,
                          struct iatt *stbuf, struct iatt *preparent,
                          struct iatt *postparent, dict_t *xdata) {
    dht_local_t *local = NULL;
...
int dht_link(call_frame_t *frame, xlator_t *this, loc_t *oldloc, loc_t *newloc,
             dict_t *xdata) {
    xlator_t *cached_subvol = NULL;
    xlator_t *hashed_subvol = NULL;
    int op_errno = -1;
...
int dht_create_linkfile_create_cbk(call_frame_t *frame, void *cookie,
                                   xlator_t *this, int32_t op_ret,
                                   int32_t op_errno, inode_t *inode,
                                   struct iatt *stbuf, struct iatt *preparent,
                                   struct iatt *postparent, dict_t *xdata) {
...
int dht_create_wind_to_avail_subvol(call_frame_t *frame, xlator_t *this,
...
int32_t dht_create_unlock_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                              int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    DHT_STACK_DESTROY(frame);
    return 0;
}

int32_t dht_create_finish(call_frame_t *frame, xlator_t *this, int op_ret,
                          int invoke_cbk) {
    dht_local_t *local = NULL, *lock_local = NULL;
    call_frame_t *lock_frame = NULL;
    int lock_count = 0;
...
int32_t dht_create_lock_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                            int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
...
int dht_create(call_frame_t *frame, xlator_t *this, loc_t *loc, int32_t flags,
...
int dht_mkdir_selfheal_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                           int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_layout_t *layout = NULL;
...
int dht_mkdir_cbk(call_frame_t *frame, void *cookie, xlator_t *this, int op_ret,
                  int op_errno, inode_t *inode, struct iatt *stbuf,
                  struct iatt *preparent, struct iatt *postparent,
                  dict_t *xdata) {
    dht_local_t *local = NULL;
...
int dht_mkdir_hashed_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                         int op_ret, int op_errno, inode_t *inode,
                         struct iatt *stbuf, struct iatt *preparent,
                         struct iatt *postparent, dict_t *xdata);

int dht_mkdir_helper(call_frame_t *frame, xlator_t *this, loc_t *loc,
                     mode_t mode, mode_t umask, dict_t *params) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
...
int dht_mkdir_hashed_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    */
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
        if (conf->subvolumes[i] == hashed_subvol) continue;
...
int dht_mkdir_guard_parent_layout_cbk(call_frame_t *frame, xlator_t *this,
...
int dht_mkdir(call_frame_t *frame, xlator_t *this, loc_t *loc, mode_t mode,
    ...
    local->mode = mode;
    local->umask = umask;
    if (params) local->params = dict_ref(params);
...
int dht_rmdir_selfheal_cbk(call_frame_t *heal_frame, void *cookie,
                           xlator_t *this, int op_ret, int op_errno,
                           dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_local_t *heal_local = NULL;
...
int dht_rmdir_hashed_subvol_cbk(call_frame_t *frame, void *cookie,
                                xlator_t *this, int op_ret, int op_errno,
                                struct iatt *preparent, struct iatt *postparent,
                                dict_t *xdata) {
    dht_local_t *local = NULL;
...
int dht_rmdir_cbk(call_frame_t *frame, void *cookie, xlator_t *this, int op_ret,
    ...
    LOCK(&frame->lock);
    {
        if (op_ret == -1) {
            if ((op_errno != ENOENT) && (op_errno != ESTALE)) {
                ...
                if (op_errno != EACCES) local->need_selfheal = 1;
            }
...
int dht_rmdir_unlock_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                         int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    DHT_STACK_DESTROY(frame);
    return 0;
}
...
int dht_rmdir_unlock(call_frame_t *frame, xlator_t *this) {
    ...
    lock_frame = copy_frame(frame);
    if (lock_frame == NULL) goto done;

    lock_local =
        dht_local_init(lock_frame, &local->loc, NULL, lock_frame->root->op);
    if (lock_local == NULL) goto done;
...
int dht_rmdir_lock_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                       int32_t op_ret, int32_t op_errno, dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
    int i = 0;
    ...
    hashed_subvol = local->hashed_subvol;
    for (i = 0; i < conf->subvolume_cnt; i++) {
        if (hashed_subvol && (hashed_subvol == conf->subvolumes[i])) continue;
...
int dht_rmdir_linkfile_unlink_cbk(call_frame_t *frame, void *cookie,
                                  xlator_t *this, int op_ret, int op_errno,
                                  struct iatt *preparent,
                                  struct iatt *postparent, dict_t *xdata) {
    dht_local_t *local = NULL;
...
int dht_rmdir_lookup_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                         int op_ret, int op_errno, inode_t *inode,
                         struct iatt *stbuf, dict_t *xattr,
                         struct iatt *parent) {
    dht_local_t *local = NULL;
...
int dht_rmdir_cached_lookup_cbk(call_frame_t *frame, void *cookie,
                                xlator_t *this, int op_ret, int op_errno,
...
int dht_rmdir_is_subvol_empty(call_frame_t *frame, xlator_t *this,
    ...
    list_for_each_entry(trav, &entries->list, list) {
        if (strcmp(trav->d_name, ".") == 0) continue;
        if (strcmp(trav->d_name, "..") == 0) continue;
...
err:
    ...
    if (lookup_frame) DHT_STACK_DESTROY(lookup_frame);
    ...
    LOCK(&frame->lock);
    {
        ...
        if (!local->call_cnt) unwind = _gf_true;
    }
    UNLOCK(&frame->lock);
...
int dht_rmdir_readdirp_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
...
int dht_rmdir_opendir_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
    ...
    if (!is_last_call(this_call_cnt)) return 0;
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
        ...
        readdirp_local =
            dht_local_init(readdirp_frame, &local->loc, local->fd, 0);
...
int dht_rmdir(call_frame_t *frame, xlator_t *this, loc_t *loc, int flags,
              dict_t *xdata) {
    dht_local_t *local = NULL;
    dht_conf_t *conf = NULL;
    int op_errno = -1;
...
int dht_entrylk_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                    int32_t op_ret, int32_t op_errno, dict_t *xdata)
 ...
 */
int dht_entrylk(call_frame_t *frame, xlator_t *this, const char *volume,
...
int dht_fentrylk_cbk(call_frame_t *frame, void *cookie, xlator_t *this,
                     int32_t op_ret, int32_t op_errno, dict_t *xdata)
...
int dht_fentrylk(call_frame_t *frame, xlator_t *this, const char *volume,
...
int32_t dht_ipc(call_frame_t *frame, xlator_t *this, int32_t op,
    ...
    if (op != GF_IPC_TARGET_UPCALL) goto wind_default;
    ...
    if (xdata) {
        if (dict_set_int8(xdata, conf->xattr_name, 0) < 0) goto err;
    }
...
int dht_notify(xlator_t *this, int event, void *data, ...) {
    ...
    switch (event) {
        ...
        case GF_EVENT_VOLUME_DEFRAG: {
            ...
            LOCK(&defrag->lock);
            {
                if (defrag->is_exiting) goto unlock;
        ...
        case GF_EVENT_UPCALL:
            ...
            if (up_data->event_type != GF_UPCALL_CACHE_INVALIDATION) break;
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
        if (!conf->last_event[i]) have_heard_from_all = 0;
    }
    ...
    ret = 0;
    if (propagate) ret = default_notify(this, event, data);
out:
    return ret;
}
...
void dht_log_new_layout_for_dir_selfheal(xlator_t *this, loc_t *loc,
    ...
    if (log_level < GF_LOG_INFO) return;

    if (!layout) return;

    if (!layout->cnt) return;
    ...
    for (i = 0; i < layout->cnt; i++) {
        ret =
            snprintf(string, max_string_len,
    ...
    for (i = 0; i < layout->cnt; i++) {
        ret =
            snprintf(output_string + off, len - off,
                     "[Subvol_name: %s, Err: %d , Start: "
                     "%" PRIu32 " , Stop: %" PRIu32 " , Hash: %" PRIu32 " ], ",
                     layout->list[i].xlator->name, layout->list[i].err,
                     layout->list[i].start, layout->list[i].stop,
...
int32_t dht_migration_get_dst_subvol(xlator_t *this, dht_local_t *local) {
    ...
    if (local->rebalance.target_node) ret = 0;
...
int32_t dht_set_local_rebalance(xlator_t *this, dht_local_t *local,
    ...
    if (stbuf) memcpy(&local->rebalance.stbuf, stbuf, sizeof(struct iatt));

    if (prebuf) memcpy(&local->rebalance.prebuf, prebuf, sizeof(struct iatt));
...
gf_boolean_t dht_is_tier_xlator(xlator_t *this) {
    if (strcmp(this->type, "cluster/tier") == 0) return _gf_true;
    return _gf_false;
}
...
out:
    if (xdata_in) dict_unref(xdata_in);
    return ret;
}
