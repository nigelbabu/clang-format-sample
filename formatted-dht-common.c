...
int dht_discover_complete (xlator_t *this, call_frame_t *discover_frame) {
    ...
    */
    ...
    if (local->cached_subvol) {
        ...
        if (ret < 0) {
            gf_msg (
                this->name, GF_LOG_WARNING, 0, DHT_MSG_LAYOUT_SET_FAILED,
    ...
    if (source && (heal_path || layout_anomalies || error_while_marking_mds)) {
        ...
        if (heal_frame) {
            ...
            ret =
                synctask_new (this->ctx->env, dht_heal_full_path,
    ...
    if (is_revalidate (loc)) {
        ...
        if (layout->gen && (layout->gen < conf->gen)) {
            gf_msg_trace (this->name, 0,
                          "incomplete layout failure for path=%s", loc->path);
        ...
        if (!local->op_ret) {
            ...
            if (local->fop == GF_FOP_SETXATTR) {
                STACK_WIND (frame, dht_common_mds_xattrop_cbk,
                            local->mds_subvol, local->mds_subvol->fops->xattrop,
                            &local->loc, GF_XATTROP_ADD_ARRAY, xattrop, NULL);
            } else {
                STACK_WIND (frame, dht_common_mds_xattrop_cbk,
                            local->mds_subvol,
...
int dht_xattrop_mds_cbk (call_frame_t *frame,
    ...
    if (local->fop == GF_FOP_SETXATTR) {
        STACK_WIND (frame, dht_setxattr_mds_cbk, local->mds_subvol,
                    local->mds_subvol->fops->setxattr, &local->loc,
                    local->xattr, local->flags, local->xattr_req);
    } else {
        STACK_WIND (frame, dht_setxattr_mds_cbk, local->mds_subvol,
...
#define DHT_IS_DIR(layout) (layout->cnt > 1)
{
    ...
    for (i = 0; i < cnt; i++) {
        subvol = layout->list[i].xlator;
        STACK_WIND (frame, dht_getxattr_cbk, subvol, subvol->fops->getxattr,
                    loc, key, xdata);
    }
    return 0;
...
int dht_nuke_dir (call_frame_t *frame,
                  xlator_t *this,
                  loc_t *loc,
                  data_t *tmp) {
...
int dht_statfs (call_frame_t *frame,
                xlator_t *this,
                loc_t *loc,
                dict_t *xdata) {
    dht_local_t *local = NULL;
        ...
        if (local->xattr) {
            ...
            if (conf->readdir_optimize == _gf_true) {
                if (xvol != local->first_up_subvol) {
                    ret =
                        dict_set_int32 (local->xattr, GF_READDIR_SKIP_DIRS, 1);
                    if (ret)
                        gf_msg (this->name, GF_LOG_ERROR, 0,
                                DHT_MSG_DICT_SET_FAILED,
                                "Failed to set "
    ...
    if (conf->decommission_subvols_cnt) {
        for (i = 0; i < conf->subvolume_cnt; i++) {
            if (conf->decommissioned_bricks[i] &&
                ...
                if (ret) {
                    gf_msg (this->name, GF_LOG_ERROR, ENOMEM,
                            DHT_MSG_LOC_FAILED, "parent loc build failed");
                    goto err;
                }
...
int dht_link_cbk (call_frame_t *frame,
    ...
    if (IS_DHT_MIGRATION_PHASE2 (stbuf)) {
        ret =
            dht_inode_ctx_get_mig_info (this, local->loc.inode, NULL, &subvol);
        if (!subvol) {
            /* Phase 2 of migration */
            ret = dht_rebalance_complete_check (this, frame);
            if (!ret)
...
int dht_create_cbk (call_frame_t *frame,
    ...
    if (ret != 0) {
        gf_msg_debug (this->name, 0,
                      "could not set preset layout for subvol %s", prev->name);
        op_ret = -1;
        op_errno = EINVAL;
        goto out;
    }
...
int dht_create (call_frame_t *frame,
    ...
    if (dht_filter_loc_subvol_key (this, loc, &local->loc, &subvol)) {
        gf_msg (this->name, GF_LOG_INFO, 0, DHT_MSG_SUBVOL_INFO,
                ...
                subvol->name, loc->path);
        STACK_WIND_COOKIE (frame, dht_create_cbk, subvol, subvol,
                           subvol->fops->create, &local->loc, flags, mode,
                           umask, fd, params);
        goto done;
    }
...
int dht_mkdir_helper (call_frame_t *frame,
    ...
    STACK_WIND_COOKIE (frame, dht_mkdir_hashed_cbk, hashed_subvol,
                       hashed_subvol, hashed_subvol->fops->mkdir, loc, mode,
                       umask, params);
...
int dht_rmdir_lock_cbk (call_frame_t *frame,
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
        ...
        STACK_WIND_COOKIE (
            frame, dht_rmdir_cbk, conf->subvolumes[i], conf->subvolumes[i],
            conf->subvolumes[i]->fops->rmdir, &local->loc, local->flags, NULL);
    }
...
void dht_log_new_layout_for_dir_selfheal (xlator_t *this,
    ...
    ret =
        snprintf (output_string, len, "Setting layout of %s with ", loc->path);
