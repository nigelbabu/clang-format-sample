...
int dht_link2(xlator_t* this, xlator_t* dst_node, call_frame_t* frame, int ret);

int dht_removexattr2(xlator_t* this,
                     xlator_t* subvol,
                     call_frame_t* frame,
                     int ret);

int dht_setxattr2(xlator_t* this,
                  xlator_t* subvol,
                  call_frame_t* frame,
                  int ret);

int dht_rmdir_readdirp_do(call_frame_t* readdirp_frame, xlator_t* this);

int dht_common_xattrop_cbk(call_frame_t* frame,
                           void* cookie,
                           xlator_t* this,
                           int32_t op_ret,
                           int32_t op_errno,
 ...
 */
int dht_read_iatt_from_xdata(xlator_t* this,
                             dict_t* xdata,
                             struct iatt* stbuf) {
  int ret = -1;
  int32_t mode = 0;
...
int dht_rmdir_unlock(call_frame_t* frame, xlator_t* this);
 ...
 */
static gf_boolean_t dht_match_xattr(const char* key) {
  return gf_get_index_by_elem(xattrs_to_heal, (char*)key) >= 0;
}

int dht_aggregate_quota_xattr(dict_t* dst, char* key, data_t* value) {
  int ret = -1;
  ...
  if (value == NULL) {
    gf_msg("dht", GF_LOG_WARNING, 0, DHT_MSG_DATA_NULL, "data value is NULL");
    ret = -1;
    goto out;
  }
  ...
  if (value->len > sizeof(int64_t)) {
    ...
    meta_dst->size = hton64(ntoh64(meta_dst->size) + ntoh64(meta_src->size));
    meta_dst->file_count =
...
static char* getChoices(const char* value) {
  int i = 0;
  char* ptr = NULL;
  char* tok = NULL;
  char* result = NULL;
...
int dht_aggregate(dict_t* this, char* key, data_t* value, void* data) {
  dict_t* dst = NULL;
  int32_t ret = -1;
  ...
  if (strcmp(key, GF_AFR_SBRAIN_STATUS) == 0) {
    ...
    if (ret) {
      gf_msg("dht", GF_LOG_WARNING, 0, DHT_MSG_AGGREGATE_QUOTA_XATTR_FAILED,
 ...
 */
int dht_inode_ctx_mdsvol_set(inode_t* inode,
                             xlator_t* this,
                             xlator_t* mds_subvol) {
...
int dht_inode_ctx_mdsvol_get(inode_t* inode,
                             xlator_t* this,
                             xlator_t** mdsvol) {
  dht_inode_ctx_t* ctx = NULL;
  int ret = -1;
...
int dht_lookup_selfheal_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int op_ret,
                            int op_errno,
  ...
  if (local->loc.parent) {
    dht_inode_ctx_time_update(local->loc.parent, this, &local->postparent, 1);
  }
...
int dht_discover_complete(xlator_t* this, call_frame_t* discover_frame) {
  dht_local_t* local = NULL;
  ...
  */
  ...
  if (local->need_xattr_heal && (local->mds_xattr)) {
    dht_dir_set_heal_xattr(this, local, local->xattr, local->mds_xattr, NULL,
                           NULL);
  ...
  if (IA_ISDIR(local->stbuf.ia_type) && !is_read_only) {
    for (i = 0; i < layout->cnt; i++) {
      ...
      if (layout->list[i].err == ENOENT || layout->list[i].err == ESTALE) {
        heal_path = 1;
      }
  ...
  if (IA_ISDIR(local->stbuf.ia_type)) {
    ...
    */
    if (!local->op_ret && !__is_root_gfid(local->stbuf.ia_gfid))
      (void)dht_common_mark_mdsxattr(discover_frame, &error_while_marking_mds,
                                     1);
...
int dht_common_mark_mdsxattr_cbk(call_frame_t* frame,
                                 void* cookie,
                                 xlator_t* this,
                                 int op_ret,
                                 int op_errno,
...
/* Common function call by revalidate/selfheal code path to populate
   ...
   penalty.
*/
int dht_common_mark_mdsxattr(call_frame_t* frame,
                             int* errst,
  ...
  */
  if (!dict_get(local->xattr, conf->mds_xattr_key)) {
    ...
    */
    if (mark_during_fresh_lookup) {
      ...
      if (vol_down) {
        gf_msg_debug(this->name, 0,
                     ...
                     " path %s gfid is %s ",
                     conf->subvolumes[i]->name, local->loc.path, gfid_local);
        goto out;
      }
    }
    ...
    if (!hashed_subvol) {
      gf_msg(this->name, GF_LOG_DEBUG, 0, DHT_MSG_HASHED_SUBVOL_GET_FAILED,
    ...
    */
    if (mark_during_fresh_lookup) {
      ...
      STACK_WIND_COOKIE(xattr_frame, dht_common_mark_mdsxattr_cbk,
                        ...
                        hashed_subvol->fops->setxattr, &local->loc, xattrs, 0,
                        NULL);
    } else {
...
int dht_discover_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
  ...
  LOCK(&frame->lock);
  {
    ...
    */
    ...
    if (op_ret == -1) {
      ...
      gf_msg_debug(this->name, op_errno, "lookup of %s on %s returned error",
    ...
    check_mds = dht_dict_get_array(xattr, conf->mds_xattr_key, mds_xattr_val, 1,
                                   &errst);
    /* save mds subvol on inode ctx */
    ret = dht_inode_ctx_mdsvol_set(local->inode, this, prev);
    if (ret) {
      gf_msg(this->name, GF_LOG_ERROR, 0, DHT_MSG_SET_INODE_CTX_FAILED,
             "Failed to set hashed subvol for %s vol is %s", local->loc.path,
             prev->name);
    }
...
int dht_discover(call_frame_t* frame, xlator_t* this, loc_t* loc) {
  int ret;
  dht_local_t* local = NULL;
  dht_conf_t* conf = NULL;
  int call_cnt = 0;
  ...
  for (i = 0; i < call_cnt; i++) {
    STACK_WIND_COOKIE(discover_frame, dht_discover_cbk, conf->subvolumes[i],
                      conf->subvolumes[i], conf->subvolumes[i]->fops->lookup,
...
*/
int32_t dht_dict_get_array(dict_t* dict,
                           char* key,
                           int32_t value[],
                           int32_t size,
                           int* errst) {
...
int dht_dir_xattr_heal(xlator_t* this, dht_local_t* local) {
  ...
  if (copy) {
    ...
    if (!copy_local) {
      gf_msg(this->name, GF_LOG_ERROR, ENOMEM, DHT_MSG_DIR_XATTR_HEAL_FAILED,
             "Memory allocation failed "
             "for path %s gfid %s ",
             local->loc.path, gfid_local);
...
int dht_lookup_dir_cbk(call_frame_t* frame,
                       void* cookie,
                       xlator_t* this,
                       int op_ret,
                       int op_errno,
...
int static is_permission_different(ia_prot_t* prot1, ia_prot_t* prot2) {
...
int dht_revalidate_cbk(call_frame_t* frame,
                       void* cookie,
                       xlator_t* this,
                       int op_ret,
                       int op_errno,
  ...
  if (!conf->vch_forced) {
    ret = dict_get_uint32(xattr, conf->commithash_xattr_name, &vol_commit_hash);
    if (ret == 0) {
      conf->vol_commit_hash = vol_commit_hash;
    }
  }
  ...
  LOCK(&frame->lock);
  {
    ...
    if (op_ret == -1) {
      ...
      if ((op_errno != ENOTCONN) && (op_errno != ENOENT) &&
          (op_errno != ESTALE)) {
        gf_msg(this->name, GF_LOG_INFO, op_errno, DHT_MSG_REVALIDATE_CBK_INFO,
    ...
    if ((!IA_ISINVAL(local->inode->ia_type)) &&
      ...
      gf_msg(this->name, GF_LOG_WARNING, 0, DHT_MSG_FILE_TYPE_MISMATCH,
             ...
             " gfid = %s",
             (stbuf->ia_type), (local->inode->ia_type), local->loc.path, gfid);
    ...
    if (is_dir) {
      ...
      if (ret >= 0) {
        if (is_greater_time(local->stbuf.ia_ctime, local->stbuf.ia_ctime_nsec,
      ...
      if (local->stbuf.ia_type != IA_INVAL) {
        if ((local->stbuf.ia_gid != stbuf->ia_gid) ||
            ...
            (__is_root_gfid(stbuf->ia_gfid) &&
             is_permission_different(&local->stbuf.ia_prot, &stbuf->ia_prot))) {
          local->need_selfheal = 1;
        }
      }
      if (!dict_get(xattr, conf->mds_xattr_key)) {
        ...
        if (ret) {
          gf_msg(this->name, GF_LOG_ERROR, 0, DHT_MSG_SET_INODE_CTX_FAILED,
                 "Failed to set MDS subvol for %s vol is %s", local->loc.path,
                 prev->name);
        }
        if ((check_mds < 0) && !errst) {
          local->mds_xattr = dict_ref(xattr);
          gf_msg_debug(this->name, 0,
                       ...
                       " path is %s and vol name is %s "
                       " gfid is %s",
                       conf->mds_xattr_key, local->loc.path, prev->name, gfid);
          local->need_xattr_heal = 1;
        }
      }
      ret = dht_layout_dir_mismatch(this, layout, prev, &local->loc, xattr);
      if (ret != 0) {
        gf_msg(this->name, GF_LOG_INFO, 0, DHT_MSG_LAYOUT_MISMATCH,
               "Mismatching layouts for %s, gfid = %s", local->loc.path, gfid);
...
out:
  ...
  if (is_last_call(this_call_cnt)) {
    if (!IA_ISDIR(local->stbuf.ia_type) &&
        ...
        (local->stbuf.ia_nlink == 1) && (conf && conf->unhashed_sticky_bit)) {
...
int dht_lookup_linkfile_create_cbk(call_frame_t* frame,
                                   void* cookie,
                                   xlator_t* this,
                                   int32_t op_ret,
                                   int32_t op_errno,
...
unwind:
  ...
  DHT_STRIP_PHASE1_FLAGS(&local->stbuf);
  DHT_STACK_UNWIND(lookup, frame, local->op_ret, local->op_errno, local->inode,
                   &local->stbuf, local->xattr, &local->postparent);
out:
  return ret;
}

int dht_lookup_unlink_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
...
int dht_lookup_unlink_of_false_linkto_cbk(call_frame_t* frame,
                                          void* cookie,
                                          xlator_t* this,
                                          int op_ret,
                                          int op_errno,
  ...
  if (is_last_call(this_call_cnt)) {
    if (op_ret == 0) {
      ...
      if (op_errno == EBUSY) {
        gf_msg(this->name, GF_LOG_ERROR, op_errno, DHT_MSG_UNLINK_FAILED,
...
int dht_lookup_unlink_stale_linkto_cbk(call_frame_t* frame,
                                       void* cookie,
                                       xlator_t* this,
                                       int op_ret,
                                       int op_errno,
...
int dht_lookup_everywhere_done(call_frame_t* frame, xlator_t* this) {
  ...
  if (!cached_subvol) {
    if (local->skip_unlink.handle_valid_link && hashed_subvol) {
      ...
      ret = dht_fill_dict_to_avoid_unlink_of_migrating_file(local->xattr_req);
      ...
      if (ret) {
        ...
        gf_msg_debug(this->name, 0,
                     ...
                     local->loc.path);
        FRAME_SU_DO(frame, dht_local_t);
        STACK_WIND(frame, dht_lookup_unlink_stale_linkto_cbk, hashed_subvol,
                   hashed_subvol->fops->unlink, &local->loc, 0,
                   local->xattr_req);
      }
  ...
  if (hashed_subvol || local->need_lookup_everywhere) {
    ...
    } else if ((local->file_count == 1) && (hashed_subvol == cached_subvol)) {
      gf_msg_debug(this->name, 0,
    ...
    if (local->skip_unlink.handle_valid_link == _gf_true) {
      if (cached_subvol == local->skip_unlink.hash_links_to) {
        if (gf_uuid_compare(local->skip_unlink.cached_gfid,
          ...
          DHT_STACK_UNWIND(lookup, frame, -1, ESTALE, NULL, NULL, NULL, NULL);
        ...
        if (local->skip_unlink.opend_fd_count == 0) {
          ret =
              dht_fill_dict_to_avoid_unlink_of_migrating_file(local->xattr_req);
...
preset_layout:
  ...
  if (found_non_linkto_on_hashed) {
    ...
    if (!layout) {
      gf_msg(this->name, GF_LOG_INFO, 0, DHT_MSG_SUBVOL_INFO,
             ...
             " gfid = %s",
             local->loc.path, (cached_subvol ? cached_subvol->name : "<nil>"),
             gfid);
    }
  ...
  gf_msg_debug(this->name, 0,
               ...
               hashed_subvol->name, local->loc.path, cached_subvol->name, gfid);
...
int dht_lookup_everywhere_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int32_t op_ret,
                              int32_t op_errno,
  ...
  gf_msg_debug(this->name, 0,
               "returned with op_ret %d and op_errno %d (%s) "
               "from subvol %s",
               op_ret, op_errno, loc->path, subvol->name);
  ...
  LOCK(&frame->lock);
  {
    ...
    is_linkfile = check_is_linkfile(inode, buf, xattr, conf->link_xattr_name);
    ...
    if (is_dir) {
      ...
      gf_msg_debug(this->name, 0, "found on %s directory %s", subvol->name,
                   loc->path);
    } else {
      local->file_count++;
      ...
      if (!local->cached_subvol) {
        /* found one file */
        dht_iatt_merge(this, &local->stbuf, buf, subvol);
        local->xattr = dict_ref(xattr);
  ...
  if (is_linkfile) {
    ...
    if (local->hashed_subvol && local->hashed_subvol == subvol) {
      ...
      if (ret) {
         ...
         */
        FRAME_SU_DO(frame, dht_local_t);
        STACK_WIND(frame, dht_lookup_unlink_cbk, subvol, subvol->fops->unlink,
                   loc, 0, dict_req);
...
int dht_lookup_linkfile_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int op_ret,
                            int op_errno,
...
xlator_t* dht_inode_get_hashed_subvol(inode_t* inode,
                                      xlator_t* this,
                                      loc_t* loc) {
  char* path = NULL;
  loc_t populate_loc = {
...
int dht_lookup_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int op_ret,
                   int op_errno,
 ...
 */
void dht_check_and_set_acl_xattr_req(inode_t* inode, dict_t* xattr_req) {
  int ret = 0;
  ...
  if (!dict_get(xattr_req, POSIX_ACL_ACCESS_XATTR)) {
    ...
    if (ret)
      gf_msg(THIS->name, GF_LOG_WARNING, -ret, DHT_MSG_DICT_SET_FAILED,
             "Failed to set dictionary value:key = %s", POSIX_ACL_ACCESS_XATTR);
  }
...
int dht_lookup(call_frame_t* frame,
               xlator_t* this,
               loc_t* loc,
               dict_t* xattr_req) {
  xlator_t* subvol = NULL;
  ...
  if (ret) {
    ...
    if (ret == -1) {
      op_errno = errno;
      gf_msg_debug(this->name, errno, "copying location failed for path=%s",
                   loc->path);
      goto err;
    }
  }
  ...
  if (is_revalidate(loc)) {
    ...
    if (layout->gen && (layout->gen < conf->gen)) {
      gf_msg_trace(this->name, 0, "incomplete layout failure for path=%s",
                   loc->path);
    ...
    if (IA_ISDIR(local->inode->ia_type)) {
      ...
      if (ret || !mds_subvol) {
        gf_msg_debug(this->name, 0, "Failed to get mds subvol for path %s",
                     local->loc.path);
      }
      ...
      for (i = 0; i < call_cnt; i++) {
        STACK_WIND_COOKIE(
            frame, dht_revalidate_cbk, conf->subvolumes[i], conf->subvolumes[i],
            conf->subvolumes[i]->fops->lookup, loc, local->xattr_req);
      }
      return 0;
    }
    ...
    /* need it for self-healing linkfiles which is
       'in-migration' state */
    ret = dict_set_uint32(local->xattr_req, GLUSTERFS_OPEN_FD_COUNT, 4);
    if (ret) {
      gf_msg(this->name, GF_LOG_WARNING, ENOMEM, DHT_MSG_DICT_SET_FAILED,
             "Failed to set dictionary value:key = %s for "
             "path %s",
...
int dht_unlink_linkfile_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int op_ret,
                            int op_errno,
...
int dht_unlink_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int op_ret,
                   int op_errno,
  ...
  LOCK(&frame->lock);
  {
    if (op_ret == -1) {
      ...
      gf_msg_debug(this->name, op_errno, "Unlink: subvolume %s returned -1",
                   prev->name);
      goto unlock;
    }
    ...
    if (local->loc.parent) {
      dht_inode_ctx_time_update(local->loc.parent, this, &local->preparent, 0);
      dht_inode_ctx_time_update(local->loc.parent, this, &local->postparent, 1);
    }
  }
unlock:
...
static int dht_common_setxattr_cbk(call_frame_t* frame,
                                   void* cookie,
                                   xlator_t* this,
                                   int32_t op_ret,
                                   int32_t op_errno,
...
static int dht_fix_layout_setxattr_cbk(call_frame_t* frame,
                                       void* cookie,
                                       xlator_t* this,
                                       int32_t op_ret,
                                       int32_t op_errno,
...
int dht_err_cbk(call_frame_t* frame,
                void* cookie,
                xlator_t* this,
                int op_ret,
                int op_errno,
...
*/
int32_t dht_dict_set_array(dict_t* dict,
                           char* key,
                           int32_t value[],
                           int32_t size) {
  int ret = -1;
...
int dht_common_mds_xattrop_cbk(call_frame_t* frame,
                               void* cookie,
                               xlator_t* this,
                               int32_t op_ret,
                               int32_t op_errno,
...
/* Code to wind a xattrop call to add 1 on current mds internal xattr
   value
*/
int dht_setxattr_non_mds_cbk(call_frame_t* frame,
                             void* cookie,
                             xlator_t* this,
                             int op_ret,
                             int op_errno,
...
int dht_setxattr_mds_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int op_ret,
                         int op_errno,
  ...
  for (i = 0; i < conf->subvolume_cnt; i++) {
    ...
    if (local->fop == GF_FOP_SETXATTR) {
      STACK_WIND(frame, dht_setxattr_non_mds_cbk, conf->subvolumes[i],
                 conf->subvolumes[i]->fops->setxattr, &local->loc, local->xattr,
                 local->flags, local->xattr_req);
    } else {
      STACK_WIND(frame, dht_setxattr_non_mds_cbk, conf->subvolumes[i],
                 conf->subvolumes[i]->fops->fsetxattr, local->fd, local->xattr,
                 local->flags, local->xattr_req);
    }
  }
...
out:
  if (local->fop == GF_FOP_SETXATTR) {
    DHT_STACK_UNWIND(setxattr, frame, local->op_ret, local->op_errno, xdata);
  } else {
    DHT_STACK_UNWIND(fsetxattr, frame, local->op_ret, local->op_errno, xdata);
  }

  return 0;
}

int dht_xattrop_mds_cbk(call_frame_t* frame,
                        void* cookie,
                        xlator_t* this,
                        int op_ret,
                        int op_errno,
...
static void fill_layout_info(dht_layout_t* layout, char* buf) {
  int i = 0;
  char tmp_buf[128] = {
      0,
  };
  ...
  for (i = 0; i < layout->cnt; i++) {
    snprintf(tmp_buf, 128, "(%s %u %u)", layout->list[i].xlator->name,
...
void dht_fill_pathinfo_xattr(xlator_t* this,
                             dht_local_t* local,
                             char* xattr_buf,
                             int32_t alloc_len,
                             int flag,
  ...
  if (flag && local->xattr_val)
    snprintf(xattr_buf, alloc_len,
             "((<" DHT_PATHINFO_HEADER "%s> %s) (%s-layout %s))", this->name,
             local->xattr_val, this->name, layout_buf);
  else if (local->xattr_val)
    snprintf(xattr_buf, alloc_len, "(<" DHT_PATHINFO_HEADER "%s> %s)",
             this->name, local->xattr_val);
  else if (flag)
    snprintf(xattr_buf, alloc_len, "(%s-layout %s)", this->name, layout_buf);
}

int dht_vgetxattr_alloc_and_fill(dht_local_t* local,
                                 dict_t* xattr,
                                 xlator_t* this,
                                 int op_errno) {
  int ret = -1;
  ...
  if (!local->xattr_val) {
    ...
    local->xattr_val =
        GF_CALLOC(local->alloc_len, sizeof(char), gf_common_mt_char);
    if (!local->xattr_val) {
      ret = -1;
      goto out;
    }
...
int dht_vgetxattr_fill_and_set(dht_local_t* local,
                               dict_t** dict,
                               xlator_t* this,
                               gf_boolean_t flag) {
  int ret = -1;
  ...
  *dict = dict_new();
  ...
  xattr_buf = GF_CALLOC(local->alloc_len, sizeof(char), gf_common_mt_char);
  if (!xattr_buf)
    goto out;
...
int dht_find_local_subvol_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int op_ret,
                              int op_errno,
...
int dht_vgetxattr_dir_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
...
int dht_vgetxattr_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int op_ret,
                      int op_errno,
...
int dht_linkinfo_getxattr_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int op_ret,
                              int op_errno,
...
int dht_mds_getxattr_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int op_ret,
                         int op_errno,
...
int dht_getxattr_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
...
int32_t dht_getxattr_unwind(call_frame_t* frame,
                            int op_ret,
                            int op_errno,
                            dict_t* dict,
                            dict_t* xdata) {
...
int dht_getxattr_get_real_filename_cbk(call_frame_t* frame,
                                       void* cookie,
                                       xlator_t* this,
                                       int op_ret,
                                       int op_errno,
  ...
  LOCK(&frame->lock);
  {
    ...
    if (op_ret == -1) {
      if (op_errno == ENODATA || op_errno == EOPNOTSUPP) {
        ...
        local->op_ret = op_ret;
        local->op_errno = op_errno;
        gf_msg(this->name, GF_LOG_WARNING, op_errno, DHT_MSG_UPGRADE_BRICKS,
               "At least "
       ...
       * from other subvol.
       */
      gf_msg(this->name, GF_LOG_WARNING, op_errno, DHT_MSG_GET_XATTR_FAILED,
...
int dht_getxattr_get_real_filename(call_frame_t* frame,
                                   xlator_t* this,
                                   loc_t* loc,
                                   const char* key,
                                   dict_t* xdata) {
...
int dht_marker_populate_args(call_frame_t* frame,
                             int type,
                             int* gauge,
                             xlator_t** subvols) {
  dht_local_t* local = NULL;
...
int dht_getxattr(call_frame_t* frame,
                 xlator_t* this,
                 loc_t* loc,
                 const char* key,
                 dict_t* xdata)
#define DHT_IS_DIR(layout) (layout->cnt > 1)
{
  ...
  if (key && DHT_IS_DIR(layout) && (!strcmp(key, GF_REBAL_FIND_LOCAL_SUBVOL))) {
    ...
    for (i = 0; i < cnt; i++) {
      STACK_WIND_COOKIE(frame, dht_find_local_subvol_cbk, conf->subvolumes[i],
                        conf->subvolumes[i],
                        conf->subvolumes[i]->fops->getxattr, loc, node_uuid_key,
                        xdata);
    }
    if (node_uuid_key)
  ...
  if (key && DHT_IS_DIR(layout) &&
      (XATTR_IS_PATHINFO(key) || (strcmp(key, GF_XATTR_NODE_UUID_KEY) == 0) ||
       (strcmp(key, GF_XATTR_LIST_NODE_UUIDS_KEY) == 0))) {
    (void)strncpy(local->xsel, key, 256);
    cnt = local->call_cnt = layout->cnt;
    for (i = 0; i < cnt; i++) {
      subvol = layout->list[i].xlator;
      STACK_WIND(frame, dht_vgetxattr_dir_cbk, subvol, subvol->fops->getxattr,
                 loc, key, xdata);
    }
    return 0;
  }
  ...
  if (key &&
      ((strcmp(key, GF_XATTR_NODE_UUID_KEY) == 0) || XATTR_IS_PATHINFO(key))) {
    cached_subvol = local->cached_subvol;
    (void)strncpy(local->xsel, key, 256);

    local->call_cnt = 1;
    STACK_WIND_COOKIE(frame, dht_vgetxattr_cbk, cached_subvol, cached_subvol,
      ...
      */
      ...
      for (i = 0; i < cnt; i++) {
        if (conf->subvolumes[i] == mds_subvol) {
          if (!conf->subvolume_status[i]) {
            gf_msg(this->name, GF_LOG_INFO, 0, DHT_MSG_HASHED_SUBVOL_DOWN,
...
int dht_fgetxattr(call_frame_t* frame,
                  xlator_t* this,
                  fd_t* fd,
                  const char* key,
                  dict_t* xdata) {
    ...
    if (!mds_subvol) {
      ...
      */
      ...
      for (i = 0; i < cnt; i++) {
        if (conf->subvolumes[i] == mds_subvol) {
          if (!conf->subvolume_status[i]) {
            gf_msg(this->name, GF_LOG_WARNING, 0, DHT_MSG_HASHED_SUBVOL_DOWN,
...
int dht_file_setxattr_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
  ...
  if ((local->fop == GF_FOP_FSETXATTR) && op_ret == -1 && (op_errno == EBADF) &&
      !(local->fd_checked)) {
...
*/
static int dht_is_user_xattr(dict_t* this,
                             char* key,
                             data_t* value,
                             void* data) {
 ...
 */
int dht_dir_common_setxattr(call_frame_t* frame,
                            xlator_t* this,
                            loc_t* loc,
                            fd_t* fd,
                            dict_t* xattr,
...

{
   ...
   */
  dict_foreach_fnmatch(xattr, "user*", dht_is_user_xattr, &uxattr_key_found);
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
      ...
      if (fd) {
        STACK_WIND_COOKIE(frame, dht_err_cbk, travvol, travvol,
                          travvol->fops->fsetxattr, fd, xattr, flags, xdata);
      } else {
        STACK_WIND_COOKIE(frame, dht_err_cbk, travvol, travvol,
                          travvol->fops->setxattr, loc, xattr, flags, xdata);
      }
    }
    ...
    */
    if (fd) {
      STACK_WIND(frame, dht_xattrop_mds_cbk, local->mds_subvol,
                 local->mds_subvol->fops->fxattrop, fd, GF_XATTROP_ADD_ARRAY,
                 xattrop, NULL);
    } else {
      STACK_WIND(frame, dht_xattrop_mds_cbk, local->mds_subvol,
                 local->mds_subvol->fops->xattrop, loc, GF_XATTROP_ADD_ARRAY,
                 xattrop, NULL);
    }
    if (xattrop)
      dict_unref(xattrop);
...
int dht_fsetxattr(call_frame_t* frame,
                  xlator_t* this,
                  fd_t* fd,
                  dict_t* xattr,
                  int flags,
  ...
  if (IA_ISDIR(fd->inode->ia_type)) {
    ...
    ret = dht_dir_common_setxattr(frame, this, NULL, fd, xattr, flags, xdata,
                                  &op_errno);
    if (ret)
      goto err;
  } else {
    ...
    if (ret) {
      gf_msg_debug(this->name, 0, "Failed to set dictionary key %s for fd=%p",
...
int dht_checking_pathinfo_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int op_ret,
                              int op_errno,
...
int dht_nuke_dir_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int32_t op_ret,
                     int32_t op_errno,
...
int dht_nuke_dir(call_frame_t* frame, xlator_t* this, loc_t* loc, data_t* tmp) {
...
int dht_setxattr(call_frame_t* frame,
                 xlator_t* this,
                 loc_t* loc,
                 dict_t* xattr,
                 int flags,
  ...
  subvol = local->cached_subvol;
  if (!subvol) {
    gf_msg_debug(this->name, 0, "no cached subvolume for path=%s", loc->path);
    op_errno = EINVAL;
    goto err;
  }
  ...
  if (tmp) {
    ...
    for (i = 0; i < conf->subvolume_cnt; i++) {
      ...
      STACK_WIND_COOKIE(frame, dht_checking_pathinfo_cbk, conf->subvolumes[i],
                        conf->subvolumes[i],
  ...
  if (tmp) {
    ...
    if (ret == 0) {
      gf_msg_debug(this->name, 0, "updating commit hash for %s from %u to %u",
    ...
    ret = dht_fix_directory_layout(frame, dht_fix_layout_setxattr_cbk, layout);
    if (ret) {
      op_errno = ENOTCONN;
      goto err;
    }
  ...
  if (tmp) {
    ...
    if (!ret && ((dir_spread <= conf->subvolume_cnt) && (dir_spread > 0))) {
      ...
      ret = dht_fix_directory_layout(frame, dht_common_setxattr_cbk, layout);
      if (ret) {
        op_errno = ENOTCONN;
        goto err;
      }
  ...
  if (IA_ISDIR(loc->inode->ia_type)) {
    local->hashed_subvol = NULL;
    ret = dht_dir_common_setxattr(frame, this, loc, NULL, xattr, flags, xdata,
                                  &op_errno);
    if (ret)
      goto err;
  } else {
...
int dht_file_removexattr_cbk(call_frame_t* frame,
                             void* cookie,
                             xlator_t* this,
                             int op_ret,
                             int op_errno,
...
int dht_removexattr_cbk(call_frame_t* frame,
                        void* cookie,
                        xlator_t* this,
                        int op_ret,
                        int op_errno,
...
unlock:
  ...
  this_call_cnt = dht_frame_return(frame);
  if (is_last_call(this_call_cnt)) {
    DHT_STACK_UNWIND(removexattr, frame, local->op_ret, local->op_errno, NULL);
  }

  return 0;
}

int dht_removexattr(call_frame_t* frame,
                    xlator_t* this,
                    loc_t* loc,
                    const char* key,
                    dict_t* xdata) {
  ...
  if (IA_ISDIR(loc->inode->ia_type)) {
    for (i = 0; i < call_cnt; i++) {
      STACK_WIND_COOKIE(frame, dht_removexattr_cbk, layout->list[i].xlator,
                        layout->list[i].xlator,
                        layout->list[i].xlator->fops->removexattr, loc, key,
                        local->xattr_req);
    }
    ...
    STACK_WIND_COOKIE(frame, dht_file_removexattr_cbk, subvol, subvol,
                      subvol->fops->removexattr, loc, key, local->xattr_req);
  }
...
int dht_fremovexattr(call_frame_t* frame,
                     xlator_t* this,
                     fd_t* fd,
                     const char* key,
                     dict_t* xdata) {
  ...
  if (IA_ISDIR(fd->inode->ia_type)) {
    for (i = 0; i < call_cnt; i++) {
      STACK_WIND_COOKIE(frame, dht_removexattr_cbk, layout->list[i].xlator,
                        layout->list[i].xlator,
                        layout->list[i].xlator->fops->fremovexattr, fd, key,
                        local->xattr_req);
    }
    ...
    STACK_WIND_COOKIE(frame, dht_file_removexattr_cbk, subvol, subvol,
                      subvol->fops->fremovexattr, fd, key, local->xattr_req);
  }
...
int dht_fd_cbk(call_frame_t* frame,
               void* cookie,
               xlator_t* this,
               int op_ret,
               int op_errno,
 ...
 * dht_normalize_stats -
 */
void dht_normalize_stats(struct statvfs* buf,
                         unsigned long bsize,
...
int dht_statfs_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int op_ret,
                   int op_errno,
...
int dht_statfs(call_frame_t* frame, xlator_t* this, loc_t* loc, dict_t* xdata) {
  dht_local_t* local = NULL;
  dht_conf_t* conf = NULL;
  int op_errno = -1;
  int i = -1;
...
int dht_opendir(call_frame_t* frame,
                xlator_t* this,
                loc_t* loc,
                fd_t* fd,
                dict_t* xdata) {
  ...
  if (ret)
    gf_msg(this->name, GF_LOG_WARNING, 0, DHT_MSG_DICT_SET_FAILED,
           "Failed to set dictionary value : key = %s", conf->link_xattr_name);
...
void dht_populate_inode_for_dentry(xlator_t* this,
                                   xlator_t* subvol,
                                   gf_dirent_t* entry,
                                   gf_dirent_t* orig_entry) {
  dht_layout_t* layout = NULL;
  int ret = 0;
  loc_t loc = {
      0,
...
int dht_readdirp_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
  ...
  list_for_each_entry(orig_entry, (&orig_entries->list), list) {
    ...
    if (check_is_dir(NULL, (&orig_entry->d_stat), NULL)) {
      ...
      hashed_subvol = methods->layout_search(this, layout, orig_entry->d_name);
  ...
  list:
    ...
    if (IA_ISDIR(entry->d_stat.ia_type)) {
      ...
      if (orig_entry->inode) {
        dht_inode_ctx_time_update(orig_entry->inode, this, &entry->d_stat, 1);

        if (conf->subvolume_cnt == 1) {
          dht_populate_inode_for_dentry(this, prev, entry, orig_entry);
        }
      }
    } else {
      if (orig_entry->inode) {
  ...
  if ((count == 0) || (local && (local->filled < local->size))) {
    ...
    STACK_WIND_COOKIE(frame, dht_readdirp_cbk, next_subvol, next_subvol,
                      ...
                      (local->size - local->filled), next_offset, local->xattr);
    return 0;
  }
...
int dht_readdir_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int op_ret,
                    int op_errno,
  ...
  if ((count == 0) || (local && (local->filled < local->size))) {
    ...
    STACK_WIND_COOKIE(frame, dht_readdir_cbk, next_subvol, next_subvol,
                      ...
                      (local->size - local->filled), next_offset, NULL);
    return 0;
  }
...
int dht_do_readdir(call_frame_t* frame,
                   xlator_t* this,
                   fd_t* fd,
                   size_t size,
                   off_t yoff,
  ...
  if (whichop == GF_FOP_READDIRP) {
    ...
    STACK_WIND_COOKIE(frame, dht_readdirp_cbk, xvol, xvol, xvol->fops->readdirp,
                      fd, size, yoff, local->xattr);
  } else {
    STACK_WIND_COOKIE(frame, dht_readdir_cbk, xvol, xvol, xvol->fops->readdir,
                      fd, size, yoff, local->xattr);
  }
...
int dht_readdir(call_frame_t* frame,
                xlator_t* this,
                fd_t* fd,
                size_t size,
                off_t yoff,
...
int dht_readdirp(call_frame_t* frame,
                 xlator_t* this,
                 fd_t* fd,
                 size_t size,
                 off_t yoff,
...
int dht_fsyncdir_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
  ...
  this_call_cnt = dht_frame_return(frame);
  if (is_last_call(this_call_cnt))
    DHT_STACK_UNWIND(fsyncdir, frame, local->op_ret, local->op_errno, xdata);

  return 0;
}

int dht_fsyncdir(call_frame_t* frame,
                 xlator_t* this,
                 fd_t* fd,
                 int datasync,
                 dict_t* xdata) {
...
int dht_newfile_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int op_ret,
                    int op_errno,
...
int dht_mknod_linkfile_create_cbk(call_frame_t* frame,
                                  void* cookie,
                                  xlator_t* this,
                                  int32_t op_ret,
                                  int32_t op_errno,
  ...
  STACK_WIND_COOKIE(frame, dht_newfile_cbk, (void*)cached_subvol, cached_subvol,
                    cached_subvol->fops->mknod, &local->loc, local->mode,
                    local->rdev, local->umask, local->params);
...
int dht_mknod_wind_to_avail_subvol(call_frame_t* frame,
                                   xlator_t* this,
                                   xlator_t* subvol,
                                   loc_t* loc,
                                   dev_t rdev,
  ...
  if (!dht_is_subvol_filled(this, subvol)) {
    gf_msg_debug(this->name, 0, "creating %s on %s", loc->path, subvol->name);
...
int32_t dht_mknod_do(call_frame_t* frame) {
  dht_local_t* local = NULL;
  ...
  dht_mknod_wind_to_avail_subvol(frame, this, subvol, &local->loc, local->rdev,
...
int32_t dht_mknod_unlock_cbk(call_frame_t* frame,
                             void* cookie,
                             xlator_t* this,
                             int32_t op_ret,
                             int32_t op_errno,
...
int32_t dht_mknod_finish(call_frame_t* frame,
                         xlator_t* this,
                         int op_ret,
                         int invoke_cbk) {
  ...
  lock_local =
      dht_local_init(lock_frame, &local->loc, NULL, lock_frame->root->op);
  if (lock_local == NULL) {
    goto done;
  }
  ...
  dht_unlock_inodelk(lock_frame, lock_local->lock[0].layout.parent_layout.locks,
                     lock_local->lock[0].layout.parent_layout.lk_count,
...
int32_t dht_mknod_lock_cbk(call_frame_t* frame,
                           void* cookie,
                           xlator_t* this,
                           int32_t op_ret,
                           int32_t op_errno,
...
int32_t dht_mknod_lock(call_frame_t* frame, xlator_t* subvol) {
  dht_local_t* local = NULL;
  int count = 1, ret = -1;
  ...
  lk_array[0] =
      dht_lock_new(frame->this, subvol, &local->loc, F_RDLCK,
                   DHT_LAYOUT_HEAL_DOMAIN, NULL, IGNORE_ENOENT_ESTALE);
...
int dht_refresh_parent_layout_resume(call_frame_t* frame,
                                     xlator_t* this,
                                     int ret,
                                     int invoke_cbk) {
...
int dht_refresh_parent_layout_done(call_frame_t* frame) {
  dht_local_t* local = NULL;
  int ret = 0;
...
int dht_handle_parent_layout_change(xlator_t* this, call_stub_t* stub) {
...
int32_t dht_call_mkdir_stub(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int32_t op_ret,
                            int32_t op_errno,
...
int32_t dht_guard_parent_layout_and_namespace(xlator_t* subvol,
                                              call_stub_t* stub) {
  dht_local_t* local = NULL;
  int ret = -1;
  loc_t* loc = NULL;
  ...
  ret =
      dict_set_bin(local->params, conf->xattr_name, parent_disk_layout, 4 * 4);
  if (ret < 0) {
    local->op_errno = -ret;
    gf_msg(this->name, GF_LOG_WARNING, local->op_errno,
           DHT_MSG_PARENT_LAYOUT_CHANGED,
...
int dht_mknod(call_frame_t* frame,
              xlator_t* this,
              loc_t* loc,
              mode_t mode,
              dev_t rdev,
...
int dht_symlink(call_frame_t* frame,
                xlator_t* this,
                const char* linkname,
                loc_t* loc,
                mode_t umask,
...
int dht_unlink(call_frame_t* frame,
               xlator_t* this,
               loc_t* loc,
               int xflag,
               dict_t* xdata) {
...
int dht_link_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int op_ret,
                 int op_errno,
  ...
  if (op_ret == -1) {
    ...
    if (local->linked) {
      ...
      if (cleanup_frame) {
        cleanup_local = dht_local_init(cleanup_frame, &local->loc2, NULL, 0);
  ...
  if (local->call_cnt != 1) {
    ...
    dht_iatt_merge(this, &local->preparent, preparent, NULL);
    dht_iatt_merge(this, &local->postparent, postparent, NULL);
    if (!stbuf_merged) {
      dht_iatt_merge(this, &local->stbuf, stbuf, NULL);
      stbuf_merged = _gf_true;
    }
...
int dht_link_linkfile_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
...
int dht_link(call_frame_t* frame,
             xlator_t* this,
             loc_t* oldloc,
             loc_t* newloc,
             dict_t* xdata) {
  ...
  if (hashed_subvol != cached_subvol) {
    ...
    STACK_WIND(frame, dht_link_cbk, cached_subvol, cached_subvol->fops->link,
               oldloc, newloc, xdata);
  }
...
int dht_create_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int op_ret,
                   int op_errno,
...
int dht_create_linkfile_create_cbk(call_frame_t* frame,
                                   void* cookie,
                                   xlator_t* this,
                                   int32_t op_ret,
                                   int32_t op_errno,
...
int dht_create_wind_to_avail_subvol(call_frame_t* frame,
                                    xlator_t* this,
                                    xlator_t* subvol,
                                    loc_t* loc,
                                    int32_t flags,
...
int dht_build_parent_loc(xlator_t* this,
                         loc_t* parent,
                         loc_t* child,
                         int32_t* op_errno) {
...
int32_t dht_create_do(call_frame_t* frame) {
  dht_local_t* local = NULL;
  dht_layout_t* refreshed = NULL;
  xlator_t* subvol = NULL;
  xlator_t* this = NULL;
...
int32_t dht_create_unlock_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int32_t op_ret,
                              int32_t op_errno,
...
int32_t dht_create_finish(call_frame_t* frame,
                          xlator_t* this,
                          int op_ret,
                          int invoke_cbk) {
  dht_local_t *local = NULL, *lock_local = NULL;
...
int32_t dht_create_lock_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int32_t op_ret,
                            int32_t op_errno,
...
int32_t dht_create_lock(call_frame_t* frame, xlator_t* subvol) {
  dht_local_t* local = NULL;
  int count = 1, ret = -1;
  dht_lock_t** lk_array = NULL;
...
int dht_create(call_frame_t* frame,
               xlator_t* this,
               loc_t* loc,
               int32_t flags,
               mode_t mode,
  ...
  if (dht_filter_loc_subvol_key(this, loc, &local->loc, &subvol)) {
    gf_msg(this->name, GF_LOG_INFO, 0, DHT_MSG_SUBVOL_INFO,
           ...
           subvol->name, loc->path);
    STACK_WIND_COOKIE(frame, dht_create_cbk, subvol, subvol,
                      subvol->fops->create, &local->loc, flags, mode, umask, fd,
                      params);
    goto done;
  }
  ...
  dht_create_wind_to_avail_subvol(frame, this, subvol, loc, flags, mode, umask,
                                  fd, params);
done:
  return 0;
...
int dht_mkdir_selfheal_cbk(call_frame_t* frame,
                           void* cookie,
                           xlator_t* this,
                           int32_t op_ret,
                           int32_t op_errno,
  ...
  DHT_STACK_UNWIND(mkdir, frame, op_ret, op_errno, local->inode, &local->stbuf,
                   &local->preparent, &local->postparent, NULL);

  return 0;
}

int dht_mkdir_cbk(call_frame_t* frame,
                  void* cookie,
                  xlator_t* this,
                  int op_ret,
                  int op_errno,
...
int dht_mkdir_hashed_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int op_ret,
                         int op_errno,
...
int dht_mkdir_helper(call_frame_t* frame,
                     xlator_t* this,
                     loc_t* loc,
                     mode_t mode,
                     mode_t umask,
...
int dht_mkdir_hashed_cbk(call_frame_t* frame,
  ...
  if (op_ret == -1) {
    ...
    parent_layout_changed =
        (xdata && dict_get(xdata, GF_PREOP_CHECK_FAILED)) ? 1 : 0;
    if (parent_layout_changed) {
      ...
      stub = fop_mkdir_stub(frame, dht_mkdir_helper, &local->loc, local->mode,
  ...
  */
  ...
  if (local->call_cnt == 0) {
    ...
    dht_unlock_namespace(frame, &local->lock[0]);
    FRAME_SU_DO(frame, dht_local_t);
    dht_selfheal_directory(frame, dht_mkdir_selfheal_cbk, &local->loc, layout);
    return 0;
  }
  ...
  for (i = 0; i < conf->subvolume_cnt; i++) {
    ...
    STACK_WIND_COOKIE(frame, dht_mkdir_cbk, conf->subvolumes[i],
                      ...
                      &local->loc, local->mode, local->umask, local->params);
  }
...
int dht_mkdir_guard_parent_layout_cbk(call_frame_t* frame,
                                      xlator_t* this,
                                      loc_t* loc,
                                      mode_t mode,
                                      mode_t umask,
...
int dht_mkdir(call_frame_t* frame,
              xlator_t* this,
              loc_t* loc,
              mode_t mode,
              mode_t umask,
  ...
  if (!params || !dict_get(params, "gfid-req")) {
    op_errno = EPERM;
    gf_msg_callingfn(this->name, GF_LOG_WARNING, op_errno, DHT_MSG_GFID_NULL,
                     "mkdir: %s is received "
                     "without gfid-req %p",
                     loc->path, params);
    goto err;
  ...
  hashed_subvol = dht_subvol_get_hashed(this, loc);
  if (hashed_subvol == NULL) {
    gf_msg_debug(this->name, 0, "hashed subvol not found for %s", loc->path);
    local->op_errno = EIO;
    goto err;
  }
  ...
  stub = fop_mkdir_stub(frame, dht_mkdir_guard_parent_layout_cbk, loc, mode,
                        umask, params);
  if (stub == NULL) {
    gf_msg(this->name, GF_LOG_WARNING, ENOMEM, DHT_MSG_PARENT_LAYOUT_CHANGED,
           "mkdir (%s/%s) (path: %s): "
...
int dht_rmdir_selfheal_cbk(call_frame_t* heal_frame,
                           void* cookie,
                           xlator_t* this,
                           int op_ret,
                           int op_errno,
...
int dht_rmdir_hashed_subvol_cbk(call_frame_t* frame,
                                void* cookie,
                                xlator_t* this,
                                int op_ret,
                                int op_errno,
  ...
  LOCK(&frame->lock);
  {
    if (op_ret == -1) {
      ...
      local->op_ret = -1;
      if (conf->subvolume_cnt != 1) {
        if (op_errno != ENOENT && op_errno != EACCES && op_errno != ESTALE) {
          local->need_selfheal = 1;
        }
      }
...
unlock:
  ...
  if (is_last_call(this_call_cnt)) {
    if (local->need_selfheal) {
      ...
      dht_selfheal_restore(heal_frame, dht_rmdir_selfheal_cbk, &heal_local->loc,
                           heal_local->layout);
      return 0;
    } else {
      if (local->loc.parent) {
        dht_inode_ctx_time_update(local->loc.parent, this, &local->preparent,
                                  0);
...
int dht_rmdir_cbk(call_frame_t* frame,
                  void* cookie,
                  xlator_t* this,
                  int op_ret,
                  int op_errno,
    ...
    if (local->need_selfheal && local->fop_succeeded) {
      ...
      if (local->op_ret == 0) {
        ...
        STACK_WIND_COOKIE(frame, dht_rmdir_hashed_subvol_cbk,
                          ...
                          local->hashed_subvol->fops->rmdir, &local->loc,
                          local->flags, NULL);
      } else {
...
int dht_rmdir_unlock_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int32_t op_ret,
                         int32_t op_errno,
...
int dht_rmdir_unlock(call_frame_t* frame, xlator_t* this) {
  ...
  lock_local->lock[0].ns.parent_layout.locks =
      local->lock[0].ns.parent_layout.locks;
...
int dht_rmdir_lock_cbk(call_frame_t* frame,
                       void* cookie,
                       xlator_t* this,
                       int32_t op_ret,
                       int32_t op_errno,
...
int dht_rmdir_do(call_frame_t* frame, xlator_t* this) {
  dht_local_t* local = NULL;
  dht_conf_t* conf = NULL;
  int ret = -1;
  xlator_t* hashed_subvol = NULL;
  ...
  if (!hashed_subvol) {
    ...
    gf_msg(this->name, GF_LOG_WARNING, 0, DHT_MSG_HASHED_SUBVOL_GET_FAILED,
           "Failed to get hashed subvol for %s (gfid = %s)", local->loc.path,
           gfid);
  } else {
    local->hashed_subvol = hashed_subvol;
  }
  ...
  if (conf->subvolume_cnt == 1) {
    STACK_WIND_COOKIE(frame, dht_rmdir_hashed_subvol_cbk, conf->subvolumes[0],
                      conf->subvolumes[0], conf->subvolumes[0]->fops->rmdir,
                      &local->loc, local->flags, NULL);
    return 0;
  }
...
int dht_rmdir_linkfile_unlink_cbk(call_frame_t* frame,
                                  void* cookie,
                                  xlator_t* this,
                                  int op_ret,
                                  int op_errno,
  ...
  if (op_ret == 0) {
    ...
    gf_msg_debug(this->name, op_errno, "Unlink of %s on %s failed. (gfid = %s)",
                 local->loc.path, src->name, gfid);
  }
...
int dht_rmdir_lookup_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int op_ret,
                         int op_errno,
  ...
  if (op_ret != 0) {
    gf_msg(this->name, GF_LOG_WARNING, op_errno, DHT_MSG_FILE_LOOKUP_FAILED,
           "lookup failed for %s on %s  (type=0%o)", local->loc.path, src->name,
           stbuf->ia_type);
    goto err;
  }
  ...
  if (!check_is_linkfile(inode, stbuf, xattr, conf->link_xattr_name)) {
    ...
    gf_msg(this->name, GF_LOG_WARNING, 0, DHT_MSG_NOT_LINK_FILE_ERROR,
           "%s on %s is not a linkfile (type=0%o, gfid = %s)", local->loc.path,
           src->name, stbuf->ia_type, gfid);
    goto err;
  }
...
int dht_rmdir_cached_lookup_cbk(call_frame_t* frame,
                                void* cookie,
                                xlator_t* this,
                                int op_ret,
                                int op_errno,
...
int dht_rmdir_is_subvol_empty(call_frame_t* frame,
                              xlator_t* this,
                              gf_dirent_t* entries,
                              xlator_t* src) {
  int ret = 0;
...
void dht_rmdir_readdirp_done(call_frame_t* readdirp_frame, xlator_t* this) {
  call_frame_t* main_frame = NULL;
...
int dht_rmdir_readdirp_cbk(call_frame_t* frame,
                           void* cookie,
                           xlator_t* this,
                           int op_ret,
                           int op_errno,
...
int dht_rmdir_opendir_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
  ...
  for (i = 0; i < conf->subvolume_cnt; i++) {
    ...
    readdirp_local = dht_local_init(readdirp_frame, &local->loc, local->fd, 0);
    ...
    STACK_WIND_COOKIE(readdirp_frame, dht_rmdir_readdirp_cbk,
                      ...
                      conf->subvolumes[i]->fops->readdirp, readdirp_local->fd,
                      4096, 0, readdirp_local->xattr);
  }
...
int dht_rmdir(call_frame_t* frame,
              xlator_t* this,
              loc_t* loc,
              int flags,
              dict_t* xdata) {
  ...
  for (i = 0; i < conf->subvolume_cnt; i++) {
    STACK_WIND_COOKIE(frame, dht_rmdir_opendir_cbk, conf->subvolumes[i],
                      conf->subvolumes[i], conf->subvolumes[i]->fops->opendir,
                      loc, local->fd, NULL);
  }
...
int dht_entrylk_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int32_t op_ret,
                    int32_t op_errno,
 ...
 */
int dht_entrylk(call_frame_t* frame,
                xlator_t* this,
                const char* volume,
                loc_t* loc,
                const char* basename,
  ...
  STACK_WIND(frame, dht_entrylk_cbk, subvol, subvol->fops->entrylk, volume, loc,
...
int dht_fentrylk_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int32_t op_ret,
                     int32_t op_errno,
...
int dht_fentrylk(call_frame_t* frame,
                 xlator_t* this,
                 const char* volume,
                 fd_t* fd,
                 const char* basename,
...
int32_t dht_ipc_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int32_t op_ret,
                    int32_t op_errno,
...
int32_t dht_ipc(call_frame_t* frame,
                xlator_t* this,
                int32_t op,
                dict_t* xdata) {
  dht_local_t* local = NULL;
...
int dht_forget(xlator_t* this, inode_t* inode) {
  uint64_t ctx_int = 0;
  dht_inode_ctx_t* ctx = NULL;
  dht_layout_t* layout = NULL;
...
int dht_notify(xlator_t* this, int event, void* data, ...) {
  xlator_t* subvol = NULL;
  int cnt = -1;
  int i = -1;
  dht_conf_t* conf = NULL;
    ...
    */
    ...
    if (conf->defrag && !run_defrag) {
      if (methods->migration_needed(this)) {
        run_defrag = 1;
        ret = gf_thread_create(&conf->defrag->th, NULL, gf_defrag_start, this,
                               "dhtdg");
        if (ret) {
          GF_FREE(conf->defrag);
          conf->defrag = NULL;
...
int dht_inode_ctx_layout_get(inode_t* inode,
                             xlator_t* this,
                             dht_layout_t** layout) {
  dht_inode_ctx_t* ctx = NULL;
  int ret = -1;
...
void dht_log_new_layout_for_dir_selfheal(xlator_t* this,
                                         loc_t* loc,
                                         dht_layout_t* layout) {
  char string[2048] = {0};
  ...
  ret =
      snprintf(string, max_string_len, "Setting layout of %s with ", loc->path);
  ...
  ret = snprintf(output_string, len, "Setting layout of %s with ", loc->path);
...
*/
int32_t dht_set_local_rebalance(xlator_t* this,
                                dht_local_t* local,
                                struct iatt* stbuf,
                                struct iatt* prebuf,
                                struct iatt* postbuf,
