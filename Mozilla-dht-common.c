...
int
dht_discover_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int op_ret,
                 int op_errno,
                 ...
                 struct iatt* postparent)
{
    ...
    LOCK(&frame->lock);
    {
        ...
        */
        ...
        check_mds = dht_dict_get_array(
          xattr, conf->mds_xattr_key, mds_xattr_val, 1, &errst);
        /* save mds subvol on inode ctx */
        ret = dht_inode_ctx_mdsvol_set(local->inode, this, prev);
        if (ret) {
            gf_msg(this->name,
...
dht_discover(call_frame_t* frame, xlator_t* this, loc_t* loc)
{
    ...
    if (__is_root_gfid(local->loc.gfid)) {
        ret = dict_set_uint32(
          local->xattr_req, conf->commithash_xattr_name, sizeof(uint32_t));
    }
    ...
    for (i = 0; i < call_cnt; i++) {
        STACK_WIND_COOKIE(discover_frame,
                          dht_discover_cbk,
                          conf->subvolumes[i],
                          conf->subvolumes[i],
...
*/
int32_t
dht_dict_get_array(dict_t* dict,
                   char* key,
                   int32_t value[],
                   int32_t size,
                   int* errst)
...
int
dht_lookup_dir_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int op_ret,
                   int op_errno,
    ...
    if (is_last_call(this_call_cnt)) {
        ...
        */
        ...
        if (local->op_ret == 0) {
            ...
            if (ret != 0) {
                gf_msg_debug(
                  this->name, 0, "fixing assignment on %s", local->loc.path);
                goto selfheal;
            }
...
selfheal:
    ...
    ret = dht_selfheal_directory(
      frame, dht_lookup_selfheal_cbk, &local->loc, layout);
out:
    return ret;
}
...
int
dht_revalidate_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int op_ret,
                   int op_errno,
                   ...
                   struct iatt* postparent)
{
    ...
    if (!conf->vch_forced) {
        ret =
          dict_get_uint32(xattr, conf->commithash_xattr_name, &vol_commit_hash);
        if (ret == 0) {
            conf->vol_commit_hash = vol_commit_hash;
        }
    }
...
int
dht_lookup_linkfile_create_cbk(call_frame_t* frame,
                               void* cookie,
                               xlator_t* this,
                               int32_t op_ret,
                               int32_t op_errno,
...
int
dht_lookup_unlink_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int op_ret,
                      int op_errno,
...
int
dht_lookup_unlink_of_false_linkto_cbk(call_frame_t* frame,
                                      void* cookie,
                                      xlator_t* this,
                                      int op_ret,
                                      int op_errno,
...
int
dht_lookup_unlink_stale_linkto_cbk(call_frame_t* frame,
                                   void* cookie,
                                   xlator_t* this,
                                   int op_ret,
                                   int op_errno,
    ...
    if (!cached_subvol) {
        ...
        if (local->skip_unlink.handle_valid_link && hashed_subvol) {
            ...
            if (ret) {
                ...
                DHT_STACK_UNWIND(
                  lookup, frame, -1, ENOENT, NULL, NULL, NULL, NULL);
            } else {
        ...
        if (local->skip_unlink.handle_valid_link == _gf_true) {
            if (cached_subvol == local->skip_unlink.hash_links_to) {
                ...
                if (gf_uuid_compare(local->skip_unlink.cached_gfid,
                    ...
                    DHT_STACK_UNWIND(
                      lookup, frame, -1, ESTALE, NULL, NULL, NULL, NULL);
                ...
                if (local->skip_unlink.opend_fd_count == 0) {
                    ...
                    if (ret) {
                        DHT_STACK_UNWIND(
                          lookup, frame, -1, EIO, NULL, NULL, NULL, NULL);
                    } else {
                        local->call_cnt = 1;
                        FRAME_SU_DO(frame, dht_local_t);
                        STACK_WIND(frame,
...
int
dht_lookup_everywhere_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int32_t op_ret,
                          int32_t op_errno,
...
int
dht_lookup_linkfile_cbk(call_frame_t* frame,
                        void* cookie,
                        xlator_t* this,
                        int op_ret,
                        int op_errno,
...
unwind:
    ...
    DHT_STACK_UNWIND(
      lookup, frame, op_ret, op_errno, inode, stbuf, xattr, postparent);
...
int
dht_lookup_cbk(call_frame_t* frame,
               void* cookie,
               xlator_t* this,
               int op_ret,
               int op_errno,
...
int
dht_unlink_linkfile_cbk(call_frame_t* frame,
                        void* cookie,
                        xlator_t* this,
                        int op_ret,
                        int op_errno,
...
int
dht_unlink_cbk(call_frame_t* frame,
               void* cookie,
               xlator_t* this,
               int op_ret,
               int op_errno,
               ...
               dict_t* xdata)
{
    ...
    LOCK(&frame->lock);
    {
        ...
        if (local->loc.parent) {
            dht_inode_ctx_time_update(
              local->loc.parent, this, &local->preparent, 0);
            dht_inode_ctx_time_update(
              local->loc.parent, this, &local->postparent, 1);
        }
    }
...
static int
dht_common_setxattr_cbk(call_frame_t* frame,
                        void* cookie,
                        xlator_t* this,
                        int32_t op_ret,
                        int32_t op_errno,
...
static int
dht_fix_layout_setxattr_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int32_t op_ret,
                            int32_t op_errno,
...
int
dht_err_cbk(call_frame_t* frame,
            void* cookie,
            xlator_t* this,
            int op_ret,
            int op_errno,
            dict_t* xdata)
{
    ...
    LOCK(&frame->lock);
    {
        if (op_ret == -1) {
            local->op_errno = op_errno;
            gf_msg_debug(
              this->name, op_errno, "subvolume %s returned -1", prev->name);
            goto unlock;
        }
...
int
dht_common_mds_xattrop_cbk(call_frame_t* frame,
                           void* cookie,
                           xlator_t* this,
                           int32_t op_ret,
                           int32_t op_errno,
                           ...
                           dict_t* xdata)
{
    ...
    if (op_ret)
        gf_msg_debug(
          this->name, op_errno, "subvolume %s returned -1", prev->this->name);
...
*/
int
dht_setxattr_non_mds_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int op_ret,
                         int op_errno,
...
int
dht_setxattr_mds_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
...
out:
    if (local->fop == GF_FOP_SETXATTR) {
        DHT_STACK_UNWIND(
          setxattr, frame, local->op_ret, local->op_errno, xdata);
    } else {
        DHT_STACK_UNWIND(
          fsetxattr, frame, local->op_ret, local->op_errno, xdata);
    }
...
int
dht_xattrop_mds_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int op_ret,
                    int op_errno,
...
void
dht_fill_pathinfo_xattr(xlator_t* this,
                        dht_local_t* local,
                        char* xattr_buf,
                        int32_t alloc_len,
                        int flag,
                        char* layout_buf)
{
    if (flag && local->xattr_val)
        ...
        snprintf(
          xattr_buf, alloc_len, "(%s-layout %s)", this->name, layout_buf);
}

int
dht_vgetxattr_alloc_and_fill(dht_local_t* local,
                             dict_t* xattr,
                             xlator_t* this,
                             int op_errno)
{
...
int
dht_vgetxattr_fill_and_set(dht_local_t* local,
                           dict_t** dict,
                           xlator_t* this,
                           gf_boolean_t flag)
{
    ...
    *dict = dict_new();
    ...
    if (XATTR_IS_PATHINFO(local->xsel)) {
        (void)dht_fill_pathinfo_xattr(
          this, local, xattr_buf, local->alloc_len, flag, layout_buf);
...
int
dht_find_local_subvol_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
...
int
dht_vgetxattr_dir_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int op_ret,
                      int op_errno,
...
int
dht_vgetxattr_cbk(call_frame_t* frame,
                  void* cookie,
                  xlator_t* this,
                  int op_ret,
                  int op_errno,
...
int
dht_linkinfo_getxattr_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
...
int
dht_mds_getxattr_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
...
out:
    DHT_STACK_UNWIND(
      getxattr, frame, local->op_ret, op_errno, local->xattr, xdata);
    return 0;
}

int
dht_getxattr_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int op_ret,
                 int op_errno,
...
out:
    if (is_last_call(this_call_cnt)) {
        ...
        DHT_STACK_UNWIND(
          getxattr, frame, local->op_ret, op_errno, local->xattr, NULL);
    }
    return 0;
}
...
int
dht_getxattr_get_real_filename_cbk(call_frame_t* frame,
                                   void* cookie,
                                   xlator_t* this,
                                   int op_ret,
                                   int op_errno,
...
int
dht_getxattr_get_real_filename(call_frame_t* frame,
                               xlator_t* this,
                               loc_t* loc,
                               const char* key,
                               dict_t* xdata)
...
int
dht_marker_populate_args(call_frame_t* frame,
                         int type,
                         int* gauge,
                         xlator_t** subvols)
{
...
int
dht_getxattr(call_frame_t* frame,
             xlator_t* this,
             loc_t* loc,
             const char* key,
             dict_t* xdata)
#define DHT_IS_DIR(layout) (layout->cnt > 1)
{
    ...
    if (!layout) {
        gf_msg(
          this->name, GF_LOG_ERROR, 0, DHT_MSG_LAYOUT_NULL, "Layout is NULL");
        op_errno = ENOENT;
        goto err;
    }
    ...
    if (key &&
        (strncmp(key,
                 GF_XATTR_GET_REAL_FILENAME_KEY,
    ...
    if (key && (strcmp(key, GF_XATTR_LINKINFO_KEY) == 0)) {
        ...
        STACK_WIND(frame,
                   dht_linkinfo_getxattr_cbk,
                   hashed_subvol,
                   hashed_subvol->fops->getxattr,
                   loc,
    ...
    if (cluster_handle_marker_getxattr(frame,
                                       loc,
                                       key,
                                       conf->vol_uuid,
                                       dht_getxattr_unwind,
...
int
dht_fgetxattr(call_frame_t* frame,
              xlator_t* this,
              fd_t* fd,
              const char* key,
              dict_t* xdata)
...
int
dht_file_setxattr_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int op_ret,
                      int op_errno,
 ...
 */
int
dht_dir_common_setxattr(call_frame_t* frame,
                        xlator_t* this,
                        loc_t* loc,
                        fd_t* fd,
                        dict_t* xattr,
...
int
dht_fsetxattr(call_frame_t* frame,
              xlator_t* this,
              fd_t* fd,
              dict_t* xattr,
              int flags,
              dict_t* xdata)
{
    ...
    if (IA_ISDIR(fd->inode->ia_type)) {
        ...
        ret = dht_dir_common_setxattr(
          frame, this, NULL, fd, xattr, flags, xdata, &op_errno);
        if (ret)
            goto err;
    } else {
        ...
        STACK_WIND_COOKIE(frame,
                          dht_file_setxattr_cbk,
                          subvol,
                          subvol,
                          subvol->fops->fsetxattr,
                          fd,
                          xattr,
                          flags,
                          local->xattr_req);
...
int
dht_checking_pathinfo_cbk(call_frame_t* frame,
                          void* cookie,
                          xlator_t* this,
                          int op_ret,
                          int op_errno,
...
err:
    DHT_STACK_UNWIND(
      setxattr, frame, (local ? local->op_ret : -1), op_errno, NULL);
    return 0;
}

int
dht_nuke_dir_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int32_t op_ret,
                 int32_t op_errno,
...
int
dht_setxattr(call_frame_t* frame,
             xlator_t* this,
             loc_t* loc,
             dict_t* xattr,
             int flags,
             dict_t* xdata)
{
    ...
    if (!subvol) {
        gf_msg_debug(
          this->name, 0, "no cached subvolume for path=%s", loc->path);
        op_errno = EINVAL;
        goto err;
    }
    ...
    if (tmp) {
        ...
        gf_msg(this->name,
               ...
               "fixing the layout of %s",
               loc->path);

        ret =
          dht_fix_directory_layout(frame, dht_fix_layout_setxattr_cbk, layout);
        if (ret) {
            op_errno = ENOTCONN;
            goto err;
        }
    ...
    if (tmp) {
        ...
        if (!ret && ((dir_spread <= conf->subvolume_cnt) && (dir_spread > 0))) {
            ...
            ret =
              dht_fix_directory_layout(frame, dht_common_setxattr_cbk, layout);
            if (ret) {
                op_errno = ENOTCONN;
                goto err;
            }
        ...
        gf_msg(this->name,
               ...
               "wrong 'directory-spread-count' value (%s)",
               value);
        op_errno = ENOTSUP;
        goto err;
    }
    ...
    if (IA_ISDIR(loc->inode->ia_type)) {
        local->hashed_subvol = NULL;
        ret = dht_dir_common_setxattr(
          frame, this, loc, NULL, xattr, flags, xdata, &op_errno);
        if (ret)
            goto err;
    } else {
...
int
dht_file_removexattr_cbk(call_frame_t* frame,
                         void* cookie,
                         xlator_t* this,
                         int op_ret,
                         int op_errno,
...
int
dht_removexattr_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int op_ret,
                    int op_errno,
...
unlock:
    ...
    if (is_last_call(this_call_cnt)) {
        DHT_STACK_UNWIND(
          removexattr, frame, local->op_ret, local->op_errno, NULL);
    }
...
int
dht_removexattr(call_frame_t* frame,
                xlator_t* this,
                loc_t* loc,
                const char* key,
                dict_t* xdata)
...
int
dht_fremovexattr(call_frame_t* frame,
                 xlator_t* this,
                 fd_t* fd,
                 const char* key,
                 dict_t* xdata)
{
    ...
    if (!local->layout) {
        gf_msg_debug(
          this->name, 0, "no layout for inode=%s", uuid_utoa(fd->inode->gfid));
        op_errno = EINVAL;
        goto err;
    }
    local->xattr_req = xdata ? dict_ref(xdata) : dict_new();
...
unlock:
    ...
    if (is_last_call(this_call_cnt))
        DHT_STACK_UNWIND(
          open, frame, local->op_ret, local->op_errno, local->fd, NULL);
 ...
 */
void
dht_normalize_stats(struct statvfs* buf,
                    unsigned long bsize,
...
int
dht_statfs_cbk(call_frame_t* frame,
               void* cookie,
               xlator_t* this,
               int op_ret,
               int op_errno,
...
int
dht_opendir(call_frame_t* frame,
            xlator_t* this,
            loc_t* loc,
            fd_t* fd,
            dict_t* xdata)
...
void
dht_populate_inode_for_dentry(xlator_t* this,
                              xlator_t* subvol,
                              gf_dirent_t* entry,
...
int
dht_readdirp_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int op_ret,
                 int op_errno,
                 ...
                 dict_t* xdata)
{
    ...
    list:
        ...
        if (IA_ISDIR(entry->d_stat.ia_type)) {
            ...
            if (orig_entry->inode) {
                dht_inode_ctx_time_update(
                  orig_entry->inode, this, &entry->d_stat, 1);

                if (conf->subvolume_cnt == 1) {
                    dht_populate_inode_for_dentry(
                      this, prev, entry, orig_entry);
                }
            }
        } else {
            if (orig_entry->inode) {
    ...
    if ((count == 0) || (local && (local->filled < local->size))) {
        ...
        STACK_WIND_COOKIE(frame,
                          ...
                          next_subvol,
                          next_subvol,
                          next_subvol->fops->readdirp,
                          local->fd,
                          (local->size - local->filled),
                          next_offset,
                          local->xattr);
        return 0;
    }
...
unwind:
    ...
    DHT_STACK_UNWIND(
      readdirp, frame, local->op_ret, local->op_errno, &local->entries, NULL);

    return 0;
}

int
dht_readdir_cbk(call_frame_t* frame,
                void* cookie,
                xlator_t* this,
                int op_ret,
                int op_errno,
...
unwind:
    ...
    DHT_STACK_UNWIND(
      readdir, frame, local->op_ret, local->op_errno, &local->entries, NULL);

    return 0;
}

int
dht_do_readdir(call_frame_t* frame,
               xlator_t* this,
               fd_t* fd,
               size_t size,
               off_t yoff,
...
int
dht_readdir(call_frame_t* frame,
            xlator_t* this,
            fd_t* fd,
            size_t size,
            off_t yoff,
...
int
dht_readdirp(call_frame_t* frame,
             xlator_t* this,
             fd_t* fd,
             size_t size,
             off_t yoff,
...
int
dht_fsyncdir_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int op_ret,
                 int op_errno,
                 dict_t* xdata)
{
    ...
    if (is_last_call(this_call_cnt))
        DHT_STACK_UNWIND(
          fsyncdir, frame, local->op_ret, local->op_errno, xdata);

    return 0;
}

int
dht_fsyncdir(call_frame_t* frame,
             xlator_t* this,
             fd_t* fd,
             int datasync,
             dict_t* xdata)
...
int
dht_newfile_cbk(call_frame_t* frame,
                void* cookie,
                xlator_t* this,
                int op_ret,
                int op_errno,
...
int
dht_mknod_linkfile_create_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int32_t op_ret,
                              int32_t op_errno,
...
err:
    if (local && local->lock[0].layout.parent_layout.locks) {
        ...
        DHT_STACK_UNWIND(
          mknod, frame, -1, op_errno, NULL, NULL, NULL, NULL, NULL);
    }
    return 0;
}

int
dht_mknod_wind_to_avail_subvol(call_frame_t* frame,
                               xlator_t* this,
                               xlator_t* subvol,
                               loc_t* loc,
                               dev_t rdev,
                               ...
                               dict_t* params)
{
    ...
    if (!dht_is_subvol_filled(this, subvol)) {
        gf_msg_debug(
          this->name, 0, "creating %s on %s", loc->path, subvol->name);
...
dht_mknod_do(call_frame_t* frame)
{
    ...
    dht_mknod_wind_to_avail_subvol(frame,
                                   this,
                                   subvol,
                                   &local->loc,
                                   local->rdev,
...
int32_t
dht_mknod_unlock_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int32_t op_ret,
                     int32_t op_errno,
...
int32_t
dht_mknod_finish(call_frame_t* frame,
                 xlator_t* this,
                 int op_ret,
                 int invoke_cbk)
{
    ...
    DHT_STACK_UNWIND(
      mknod, frame, op_ret, local->op_errno, NULL, NULL, NULL, NULL, NULL);
    return 0;
}

int32_t
dht_mknod_lock_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int32_t op_ret,
                   int32_t op_errno,
...
dht_mknod_lock(call_frame_t* frame, xlator_t* subvol)
{
    ...
    lk_array[0] = dht_lock_new(frame->this,
                               subvol,
                               &local->loc,
                               F_RDLCK,
...
int
dht_refresh_parent_layout_resume(call_frame_t* frame,
                                 xlator_t* this,
                                 int ret,
                                 int invoke_cbk)
{
...
dht_refresh_parent_layout_done(call_frame_t* frame)
{
    ...
    dht_layout_set(
      frame->this, local->loc.inode, local->selfheal.refreshed_layout);
...
int32_t
dht_call_mkdir_stub(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int32_t op_ret,
                    int32_t op_errno,
...
dht_guard_parent_layout_and_namespace(xlator_t* subvol, call_stub_t* stub)
{
    ...
    ret = dht_disk_layout_extract_for_subvol(
      this, parent_layout, hashed_subvol, &parent_disk_layout);
    if (ret == -1) {
        local->op_errno = EINVAL;
        gf_msg(this->name,
               GF_LOG_WARNING,
    ...
    memcpy((void*)local->parent_disk_layout,
           (void*)parent_disk_layout,
    ...
    ret = dht_protect_namespace(
      frame, loc, hashed_subvol, &local->current->ns, dht_call_mkdir_stub);
    if (ret < 0)
        goto err;
          ...
          dict_t* params)
{
    ...
    if (!subvol) {
        gf_msg_debug(
          this->name, 0, "no subvolume in layout for path=%s", loc->path);
        op_errno = EIO;
        goto err;
    }
    ...
    dht_mknod_wind_to_avail_subvol(
      frame, this, subvol, loc, rdev, mode, umask, params);
...
int
dht_symlink(call_frame_t* frame,
            xlator_t* this,
            const char* linkname,
            loc_t* loc,
            mode_t umask,
...
int
dht_link_cbk(call_frame_t* frame,
             void* cookie,
             xlator_t* this,
             int op_ret,
             int op_errno,
...
out:
    ...
    dht_set_fixed_dir_stat(postparent);
    DHT_STACK_UNWIND(
      link, frame, op_ret, op_errno, inode, stbuf, preparent, postparent, NULL);
...
int
dht_link_linkfile_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int op_ret,
                      int op_errno,
         ...
         dict_t* xdata)
{
    ...
    if (!cached_subvol) {
        gf_msg_debug(
          this->name, 0, "no cached subvolume for path=%s", oldloc->path);
        op_errno = ENOENT;
        goto err;
    }
    ...
    if (!hashed_subvol) {
        gf_msg_debug(
          this->name, 0, "no subvolume in layout for path=%s", newloc->path);
        op_errno = EIO;
        goto err;
    }
...
int
dht_create_cbk(call_frame_t* frame,
               void* cookie,
               xlator_t* this,
               int op_ret,
               int op_errno,
...
int
dht_create_linkfile_create_cbk(call_frame_t* frame,
                               void* cookie,
                               xlator_t* this,
                               int32_t op_ret,
                               int32_t op_errno,
...
err:
    if (local && local->lock[0].layout.parent_layout.locks) {
        ...
        DHT_STACK_UNWIND(
          create, frame, -1, op_errno, NULL, NULL, NULL, NULL, NULL, NULL);
    }
    return 0;
}

int
dht_create_wind_to_avail_subvol(call_frame_t* frame,
                                xlator_t* this,
                                xlator_t* subvol,
                                loc_t* loc,
                                int32_t flags,
...
int
dht_build_parent_loc(xlator_t* this,
                     loc_t* parent,
                     loc_t* child,
                     int32_t* op_errno)
{
...
dht_create_do(call_frame_t* frame)
{
    ...
    dht_create_wind_to_avail_subvol(frame,
                                    this,
                                    subvol,
                                    &local->loc,
                                    local->flags,
...
int32_t
dht_create_unlock_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int32_t op_ret,
                      int32_t op_errno,
...
int32_t
dht_create_finish(call_frame_t* frame,
                  xlator_t* this,
                  int op_ret,
                  int invoke_cbk)
{
...
int32_t
dht_create_lock_cbk(call_frame_t* frame,
                    void* cookie,
                    xlator_t* this,
                    int32_t op_ret,
                    int32_t op_errno,
           ...
           dict_t* params)
{
    ...
    dht_create_wind_to_avail_subvol(
      frame, this, subvol, loc, flags, mode, umask, fd, params);
done:
    return 0;
...
int
dht_mkdir_selfheal_cbk(call_frame_t* frame,
                       void* cookie,
                       xlator_t* this,
                       int32_t op_ret,
                       int32_t op_errno,
...
int
dht_mkdir_cbk(call_frame_t* frame,
              void* cookie,
              xlator_t* this,
              int op_ret,
              int op_errno,
...
int
dht_mkdir_hashed_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
...
int
dht_mkdir_helper(call_frame_t* frame,
                 xlator_t* this,
                 loc_t* loc,
                 mode_t mode,
                 mode_t umask,
                 dict_t* params)
{
    ...
    if (memcmp(local->parent_disk_layout,
               parent_disk_layout,
    ...
    */
    ...
    if (local->call_cnt == 0) {
        ...
        dht_selfheal_directory(
          frame, dht_mkdir_selfheal_cbk, &local->loc, layout);
        return 0;
    }
...
int
dht_mkdir_guard_parent_layout_cbk(call_frame_t* frame,
                                  xlator_t* this,
                                  loc_t* loc,
                                  mode_t mode,
                                  mode_t umask,
...
err:
    DHT_STACK_UNWIND(
      mkdir, frame, -1, local->op_errno, NULL, NULL, NULL, NULL, NULL);

    return 0;
}

int
dht_mkdir(call_frame_t* frame,
          xlator_t* this,
          loc_t* loc,
          mode_t mode,
          mode_t umask,
          dict_t* params)
{
    ...
    if (hashed_subvol == NULL) {
        gf_msg_debug(
          this->name, 0, "hashed subvol not found for %s", loc->path);
        local->op_errno = EIO;
        goto err;
    }
    ...
    stub = fop_mkdir_stub(
      frame, dht_mkdir_guard_parent_layout_cbk, loc, mode, umask, params);
    if (stub == NULL) {
        gf_msg(this->name,
               GF_LOG_WARNING,
               ENOMEM,
...
int
dht_rmdir_selfheal_cbk(call_frame_t* heal_frame,
                       void* cookie,
                       xlator_t* this,
                       int op_ret,
                       int op_errno,
...
int
dht_rmdir_hashed_subvol_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int op_ret,
                            int op_errno,
...
err:
    DHT_STACK_UNWIND(
      rmdir, frame, local->op_ret, local->op_errno, NULL, NULL, NULL);
    return 0;
}

int
dht_rmdir_cbk(call_frame_t* frame,
              void* cookie,
              xlator_t* this,
              int op_ret,
              int op_errno,
...
int
dht_rmdir_unlock_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int32_t op_ret,
                     int32_t op_errno,
...
int
dht_rmdir_lock_cbk(call_frame_t* frame,
                   void* cookie,
                   xlator_t* this,
                   int32_t op_ret,
                   int32_t op_errno,
...
dht_rmdir_do(call_frame_t* frame, xlator_t* this)
{
    ...
    local->current = &local->lock[0];
    ret = dht_protect_namespace(frame,
                                &local->loc,
                                local->hashed_subvol,
                                &local->current->ns,
                                dht_rmdir_lock_cbk);
...
int
dht_rmdir_linkfile_unlink_cbk(call_frame_t* frame,
                              void* cookie,
                              xlator_t* this,
                              int op_ret,
                              int op_errno,
...
int
dht_rmdir_lookup_cbk(call_frame_t* frame,
                     void* cookie,
                     xlator_t* this,
                     int op_ret,
                     int op_errno,
...
int
dht_rmdir_cached_lookup_cbk(call_frame_t* frame,
                            void* cookie,
                            xlator_t* this,
                            int op_ret,
                            int op_errno,
...
int
dht_rmdir_is_subvol_empty(call_frame_t* frame,
                          xlator_t* this,
                          gf_dirent_t* entries,
                          xlator_t* src)
{
    ...
    list_for_each_entry(trav, &entries->list, list)
    {
        ...
        if (strcmp(trav->d_name, "..") == 0)
            continue;
        if (check_is_linkfile(
              NULL, (&trav->d_stat), trav->dict, conf->link_xattr_name)) {
            count++;
            continue;
        }
    ...
    list_for_each_entry(trav, &entries->list, list)
    {
        ...
        build_ret = dht_build_child_loc(
          this, &lookup_local->loc, &local->loc, trav->d_name);
        if (build_ret != 0)
            goto err;
        ...
        if (!subvol) {
            ...
            STACK_WIND_COOKIE(lookup_frame,
                              dht_rmdir_lookup_cbk,
                              src,
                              src,
                              src->fops->lookup,
            ...
            STACK_WIND(lookup_frame,
                       dht_rmdir_cached_lookup_cbk,
                       subvol,
                       subvol->fops->lookup,
                       &lookup_local->loc,
                       xattrs);
...
int
dht_rmdir_readdirp_cbk(call_frame_t* frame,
                       void* cookie,
                       xlator_t* this,
                       int op_ret,
                       int op_errno,
...
dht_rmdir_readdirp_do(call_frame_t* readdirp_frame, xlator_t* this)
{
    ...
    STACK_WIND_COOKIE(readdirp_frame,
...
int
dht_rmdir_opendir_cbk(call_frame_t* frame,
                      void* cookie,
                      xlator_t* this,
                      int op_ret,
                      int op_errno,
...
int
dht_entrylk_cbk(call_frame_t* frame,
                void* cookie,
                xlator_t* this,
                int32_t op_ret,
                int32_t op_errno,
 ...
 */
int
dht_entrylk(call_frame_t* frame,
            xlator_t* this,
            const char* volume,
            loc_t* loc,
            const char* basename,
...
int
dht_fentrylk_cbk(call_frame_t* frame,
                 void* cookie,
                 xlator_t* this,
                 int32_t op_ret,
                 int32_t op_errno,
...
int
dht_fentrylk(call_frame_t* frame,
             xlator_t* this,
             const char* volume,
             fd_t* fd,
             const char* basename,
...
int32_t
dht_ipc_cbk(call_frame_t* frame,
            void* cookie,
            xlator_t* this,
            int32_t op_ret,
            int32_t op_errno,
        ...
        */
        ...
        if (conf->defrag && !run_defrag) {
            if (methods->migration_needed(this)) {
                run_defrag = 1;
                ret = gf_thread_create(
                  &conf->defrag->th, NULL, gf_defrag_start, this, "dhtdg");
                if (ret) {
                    GF_FREE(conf->defrag);
                    conf->defrag = NULL;
                    kill(getpid(), SIGTERM);
...
void
dht_log_new_layout_for_dir_selfheal(xlator_t* this,
                                    loc_t* loc,
                                    dht_layout_t* layout)
{
...
*/
int32_t
dht_set_local_rebalance(xlator_t* this,
                        dht_local_t* local,
                        struct iatt* stbuf,
                        struct iatt* prebuf,
                        struct iatt* postbuf,
