...
int dht_aggregate_quota_xattr(dict_t* dst, char* key, data_t* value)
{
    ...
    if (value->len > sizeof(int64_t)) {
        ...
        meta_dst->file_count = hton64(ntoh64(meta_dst->file_count) + ntoh64(meta_src->file_count));
...
int dht_aggregate_split_brain_xattr(dict_t* dst, char* key, data_t* value)
{
    ...
    if (oldvalue && (strstr(oldvalue, "metadata-split-brain:yes"))
        ...
        if (strstr(value->data, "yes") && (strncmp(oldvalue, value->data, strlen(oldvalue)))) {
...
*/
int dht_inode_ctx_mdsvol_set(inode_t* inode, xlator_t* this, xlator_t* mds_subvol)
{
...
int dht_lookup_selfheal_cbk(call_frame_t* frame, void* cookie,
    xlator_t* this,
...
/* Common function call by revalidate/selfheal code path to populate
   ...
   penalty.
*/
int dht_common_mark_mdsxattr(call_frame_t* frame, int* errst, int mark_during_fresh_lookup)
{
    dht_local_t* local = NULL;
    xlator_t* this = NULL;
...
unlock:
    ...
    if (is_last_call(this_call_cnt)) {
        ...
        if (local->op_ret == 0) {
            ...
            dht_layout_set(this, local->inode, layout);
            if (!dict_get(local->xattr, conf->mds_xattr_key) || local->need_xattr_heal)
                goto selfheal;
        }
...
{
    if ((prot1->owner.read != prot2->owner.read) || (prot1->owner.write != prot2->owner.write) || (prot1->owner.exec != prot2->owner.exec) || (prot1->group.read != prot2->group.read) || (prot1->group.write != prot2->group.write) || (prot1->group.exec != prot2->group.exec) || (prot1->other.read != prot2->other.read) || (prot1->other.write != prot2->other.write) || (prot1->other.exec != prot2->other.exec) || (prot1->suid != prot2->suid) || (prot1->sgid != prot2->sgid) || (prot1->sticky != prot2->sticky)) {
        return 1;
    } else {
        return 0;
    }
    ...
    struct iatt* postparent)
{
    ...
    LOCK(&frame->lock);
    {
        ...
        if ((!IA_ISINVAL(local->inode->ia_type)) && stbuf->ia_type != local->inode->ia_type) {
            gf_msg(this->name, GF_LOG_WARNING, 0,
                DHT_MSG_FILE_TYPE_MISMATCH,
        ...
        if (is_dir) {
            ...
            if (local->stbuf.ia_type != IA_INVAL) {
                if ((local->stbuf.ia_gid != stbuf->ia_gid) || (local->stbuf.ia_uid != stbuf->ia_uid) || (__is_root_gfid(stbuf->ia_gfid) && is_permission_different(&local->stbuf.ia_prot, &stbuf->ia_prot))) {
                    local->need_selfheal = 1;
                }
            }
            if (!dict_get(xattr, conf->mds_xattr_key)) {
        ...
        if (!is_dir || (is_dir && dht_dir_has_layout(xattr, conf->xattr_name) >= 0)
        ...
        if (local->skip_unlink.handle_valid_link == _gf_true) {
            if (cached_subvol == local->skip_unlink.hash_links_to) {
                ...
                gf_msg_debug(this->name, 0,
                    ...
                    "path %s",
                    (local->skip_unlink.hash_links_to ? local->skip_unlink.hash_links_to->name : " <nil>"), local->loc.path);
    ...
    if (found_non_linkto_on_hashed) {
        ...
        if (!layout) {
            gf_msg(this->name, GF_LOG_INFO, 0,
                ...
                " gfid = %s",
                local->loc.path, (cached_subvol ? cached_subvol->name : "<nil>"), gfid);
        }
    ...
    if (ENTRY_MISSING(op_ret, op_errno)) {
        ...
        if (!conf->defrag && conf->lookup_optimize && loc->parent) {
            ret = dht_inode_ctx_layout_get(loc->parent, this,
                &parent_layout);
            if (ret || !parent_layout || (parent_layout->commit_hash != conf->vol_commit_hash)) {
                gf_msg_debug(this->name, 0,
            ...
            if ((conf->search_unhashed == GF_DHT_LOOKUP_UNHASHED_AUTO) && (loc->parent)) {
...
int dht_lookup(call_frame_t* frame, xlator_t* this,
    loc_t* loc, dict_t* xattr_req)
{
    ...
    if (gf_uuid_is_null(loc->pargfid) && !gf_uuid_is_null(loc->gfid) && !__is_root_gfid(loc->inode->gfid)) {
        local->cached_subvol = NULL;
...
int dht_err_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno, dict_t* xdata)
{
    dht_local_t* local = NULL;
    int this_call_cnt = 0;
...
int dht_common_mds_xattrop_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
...
int dht_vgetxattr_alloc_and_fill(dht_local_t* local, dict_t* xattr, xlator_t* this,
    int op_errno)
{
    int ret = -1;
    char* value = NULL;
...
int dht_vgetxattr_fill_and_set(dht_local_t* local, dict_t** dict, xlator_t* this,
    gf_boolean_t flag)
{
    int ret = -1;
    char* xattr_buf = NULL;
...
int dht_getxattr(call_frame_t* frame, xlator_t* this,
    loc_t* loc, const char* key, dict_t* xdata)
#define DHT_IS_DIR(layout) (layout->cnt > 1)
{
    ...
    if (key && (strncmp(key, GF_XATTR_GET_REAL_FILENAME_KEY, strlen(GF_XATTR_GET_REAL_FILENAME_KEY)) == 0)
        && DHT_IS_DIR(layout)) {
    ...
    if (key && DHT_IS_DIR(layout) && (!strcmp(key, GF_REBAL_OLD_FIND_LOCAL_SUBVOL))) {
    ...
    if (key && DHT_IS_DIR(layout) && (XATTR_IS_PATHINFO(key) || (strcmp(key, GF_XATTR_NODE_UUID_KEY) == 0) || (strcmp(key, GF_XATTR_LIST_NODE_UUIDS_KEY) == 0))) {
    ...
    if (key && ((strcmp(key, GF_XATTR_NODE_UUID_KEY) == 0) || XATTR_IS_PATHINFO(key))) {
        cached_subvol = local->cached_subvol;
        (void)strncpy(local->xsel, key, 256);
...
int dht_fgetxattr(call_frame_t* frame, xlator_t* this,
    ...
    int op_ret, int op_errno, dict_t* xdata)
{
    ...
    if ((local->fop == GF_FOP_FSETXATTR) && op_ret == -1 && (op_errno == EBADF) && !(local->fd_checked)) {
...
int dht_fsetxattr(call_frame_t* frame, xlator_t* this,
    fd_t* fd, dict_t* xattr, int flags, dict_t* xdata)
{
    xlator_t* subvol = NULL;
    dht_local_t* local = NULL;
...
int dht_setxattr(call_frame_t* frame, xlator_t* this,
    ...
    int op_ret, int op_errno, dict_t* xdata)
{
    ...
    if ((local->fop == GF_FOP_FREMOVEXATTR) && (op_ret == -1) && (op_errno == EBADF) && !(local->fd_checked)) {
        ret = dht_check_and_open_fd_on_subvol(this, frame);
        if (ret)
            goto out;
        return 0;
...
int dht_removexattr(call_frame_t* frame, xlator_t* this,
    loc_t* loc, const char* key, dict_t* xdata)
{
    xlator_t* subvol = NULL;
    int op_errno = -1;
...
int dht_fremovexattr(call_frame_t* frame, xlator_t* this,
    fd_t* fd, const char* key, dict_t* xdata)
{
    xlator_t* subvol = NULL;
    int op_errno = -1;
...
int dht_fd_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
...
int dht_readdirp_cbk(call_frame_t* frame, void* cookie, xlator_t* this, int op_ret,
...
int dht_fsyncdir(call_frame_t* frame, xlator_t* this, fd_t* fd,
...
int dht_refresh_parent_layout_resume(call_frame_t* frame, xlator_t* this, int ret,
    int invoke_cbk)
{
...
int dht_mknod(call_frame_t* frame, xlator_t* this,
    loc_t* loc, mode_t mode, dev_t rdev, mode_t umask, dict_t* params)
{
    ...
    if (conf->decommission_subvols_cnt) {
        for (i = 0; i < conf->subvolume_cnt; i++) {
            if (conf->decommissioned_bricks[i] && conf->decommissioned_bricks[i] == subvol) {
...
int dht_symlink(call_frame_t* frame, xlator_t* this,
...
int dht_link_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno,
    inode_t* inode, struct iatt* stbuf, struct iatt* preparent,
    struct iatt* postparent, dict_t* xdata)
{
    ...
    if (local->linked == _gf_true && ((local->call_cnt == 1 && !IS_DHT_MIGRATION_PHASE2(stbuf)) || (local->call_cnt != 1 && IS_DHT_MIGRATION_PHASE2(&local->stbuf)))) {
...
int dht_link(call_frame_t* frame, xlator_t* this,
...
int dht_create(call_frame_t* frame, xlator_t* this,
...
int dht_mkdir_selfheal_cbk(call_frame_t* frame, void* cookie,
    xlator_t* this,
    int32_t op_ret, int32_t op_errno, dict_t* xdata)
{
    dht_local_t* local = NULL;
...
int dht_mkdir_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
...
int dht_mkdir_hashed_cbk(call_frame_t* frame, void* cookie,
    xlator_t* this, int op_ret, int op_errno,
    inode_t* inode, struct iatt* stbuf,
    struct iatt* preparent, struct iatt* postparent,
    dict_t* xdata);

int dht_mkdir_helper(call_frame_t* frame, xlator_t* this,
...
int dht_mkdir(call_frame_t* frame, xlator_t* this,
    loc_t* loc, mode_t mode, mode_t umask, dict_t* params)
{
    dht_local_t* local = NULL;
    dht_conf_t* conf = NULL;
...
int dht_rmdir_selfheal_cbk(call_frame_t* heal_frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno, dict_t* xdata)
{
    dht_local_t* local = NULL;
    dht_local_t* heal_local = NULL;
...
int dht_rmdir_hashed_subvol_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno, struct iatt* preparent,
    struct iatt* postparent, dict_t* xdata)
{
    dht_local_t* local = NULL;
...
int dht_rmdir_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno, struct iatt* preparent,
    struct iatt* postparent, dict_t* xdata)
{
    dht_local_t* local = NULL;
...
int dht_rmdir_linkfile_unlink_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno, struct iatt* preparent,
    struct iatt* postparent, dict_t* xdata)
{
    dht_local_t* local = NULL;
...
int dht_rmdir_cached_lookup_cbk(call_frame_t* frame, void* cookie, xlator_t* this,
    int op_ret, int op_errno, inode_t* inode,
...
int dht_entrylk_cbk(call_frame_t* frame, void* cookie,
 ...
 */
int dht_entrylk(call_frame_t* frame, xlator_t* this,
...
int dht_fentrylk_cbk(call_frame_t* frame, void* cookie,
    xlator_t* this, int32_t op_ret, int32_t op_errno, dict_t* xdata)
...
int dht_fentrylk(call_frame_t* frame, xlator_t* this,
    ...
    case GF_EVENT_VOLUME_DEFRAG: {
        ...
        LOCK(&defrag->lock);
        {
            if (defrag->is_exiting)
                goto unlock;
            if ((cmd == GF_DEFRAG_CMD_STATUS) || (cmd == GF_DEFRAG_CMD_STATUS_TIER) || (cmd == GF_DEFRAG_CMD_DETACH_STATUS))
            ...
            else if (cmd == GF_DEFRAG_CMD_STOP || cmd == GF_DEFRAG_CMD_STOP_DETACH_TIER || cmd == GF_DEFRAG_CMD_DETACH_STOP)
                gf_defrag_stop(conf,
...
int dht_inode_ctx_layout_get(inode_t* inode, xlator_t* this, dht_layout_t** layout)
{
    dht_inode_ctx_t* ctx = NULL;
    int ret = -1;
...
int32_t dht_migration_needed(xlator_t* this)
{
    ...
    if ((defrag->cmd != GF_DEFRAG_CMD_START_TIER) && (defrag->cmd != GF_DEFRAG_CMD_START_DETACH_TIER))
        ret = 1;
